...
Extending the support of FlashAttention for large headdim is meaningful in the context of DeepSeek MLA. For example, when FlashAttention supports headdim values greater than 512 (such as 512, 576, etc.), we can achieve fully Fused Multi-head latent Attention (MLA) in both the prefill and decode stages. This is possible after the weight matrices W_UK and W_UV are absorbed into W_Q and W_O, resulting in C_kv with `dc >= 512` and C_q with `dc' >=512`. Namely, resulting in Attention with large headdim. TODO list👇:
