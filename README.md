...
- [2026/08] Support [FP8 Attention](./csrc/cuffpa/cute/fp8/) for headdim 64~1024 ([sm_120](./csrc/cuffpa/cute/fp8/sm_120/), forward only), achieving **580 TOPS** (D=128) on NVIDIA RTX 5090, with almost the same accuracy as SageAttention. Support per-block(QKV)/per-thread(QK)/per-channel(V) FP8 quantization and hybrid FP8/FP16 attention. 🎉🎉 Benchmarks are provided under [bench_fp8](./bench/bench_fp8.py).
