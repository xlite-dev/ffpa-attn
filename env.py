...
class ENV(object):
  ...
  def _render_bwd_decls_header(headdims):
    ...
    for d in headdims:
      ...
      if d <= 512:
        lines.append(
          f"void ffpa_attn_bwd_persistent_kv_fp16f32_d{d}(torch::Tensor Q, torch::Tensor K, "
          f"torch::Tensor V, torch::Tensor O, torch::Tensor softmax_lse, "
          f"torch::Tensor dO, torch::Tensor dQ, torch::Tensor dK, torch::Tensor dV, "
          f"int stages, int causal, double softmax_scale);"
        )
        lines.append(
          f"void ffpa_attn_bwd_persistent_kv_bf16f32_d{d}(torch::Tensor Q, torch::Tensor K, "
          f"torch::Tensor V, torch::Tensor O, torch::Tensor softmax_lse, "
          f"torch::Tensor dO, torch::Tensor dQ, torch::Tensor dK, torch::Tensor dV, "
          f"int stages, int causal, double softmax_scale);"
        )
  ...
  def _render_bwd_per_headdim_fp16_tu(cls, d: int) -> str:
    ...
    if d <= 512:
      body += "\n"
      body += cls._render_bwd_entry(
        d,
        f"ffpa_attn_bwd_persistent_kv_fp16f32_d{d}",
        "__half",
        "launch_ffpa_attn_bwd_persistent_kv_template",
        allow_stage3=False,
      )
    return body
  ...
  def _render_bwd_per_headdim_bf16_tu(cls, d: int) -> str:
    ...
    if d <= 512:
      ...
      body += cls._render_bwd_entry(
        d,
        f"ffpa_attn_bwd_persistent_kv_bf16f32_d{d}",
        "__nv_bfloat16",
        "launch_ffpa_attn_bwd_persistent_kv_template",
        allow_stage3=False,
      )
  ...
  def _render_bwd_dispatch_tu(headdims) -> str:
    ...
    persistent_headdims = [d for d in headdims if d <= 512]
    ...
    return (
      ...
      _fn("ffpa_attn_bwd_persistent_kv_fp16f32", "ffpa_attn_bwd_persistent_kv_fp16f32", "torch::kHalf").replace(
        "switch (d) {\n" + _cases("ffpa_attn_bwd_persistent_kv_fp16f32"), "switch (d) {\n" + "\n".join(
          f"    case {d}: ffpa_attn_bwd_persistent_kv_fp16f32_d{d}"
          "(Q, K, V, O, softmax_lse, dO, dQ, dK, dV, stages, causal, softmax_scale); break;"
          for d in persistent_headdims
        )
      ) + "\n" +
      _fn("ffpa_attn_bwd_persistent_kv_bf16f32", "ffpa_attn_bwd_persistent_kv_bf16f32", "torch::kBFloat16").replace(
        "switch (d) {\n" + _cases("ffpa_attn_bwd_persistent_kv_bf16f32"), "switch (d) {\n" + "\n".join(
          f"    case {d}: ffpa_attn_bwd_persistent_kv_bf16f32_d{d}"
          "(Q, K, V, O, softmax_lse, dO, dQ, dK, dV, stages, causal, softmax_scale); break;"
          for d in persistent_headdims
        )
      )
