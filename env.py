...
class ENV(object):
  ...
  def generate_split_headdim_sources(cls, build_pkg: bool = False):
    ...
    - ``fwd_<variant>_hdim{d}.cu``: lightweight wrapper TU. Includes only
      ``fwd_decls.h`` (NOT ``launch/router.cuh``); dispatches on ``stages``
    ...
    - ``fwd_<variant>_hdim{d}_s{s}.cu``: dispatcher TU. Includes
      ``launch/router.cuh`` and contains a single
      ``launch_ffpa_attn_fwd_template``
    ...
    - ``fwd_<variant>_native_hdim{d}_s{s}.cu``: native family TU
      (``dispatch/native_fp16.cuh``): explicit instantiation of
    ...
    - ``fwd_<dtype>_fp8_hdim{d}_s{s}.cu`` (ENABLE_FFPA_TMA_EXT): fp8
      family TU (``dispatch/cute_fp8.cuh``): explicit instantiation of
      ``ffpa_fwd_fp8``.
    - ``fwd_<dtype>_fp4_hdim{d}.cu`` (ENABLE_FFPA_TMA_EXT): fp4 family
      TU (``dispatch/cute_fp4.cuh``): a single TU per (dtype, d)
      instantiating
  ...
  def _render_stage_tu(
    ...
    Only includes ``launch/router.cuh`` (routing); the family entry symbols
  ...
  # (cute_fp16/fp8/fp4). fp16f16 and fp16f32 share the __half TU.
  _DTYPE_TOKENS = {
    "__half": "fp16",
    "__nv_bfloat16": "bf16",
  }
  ...
  def _iter_generated_tus(cls, headdims):
    ...
    QK/PV constexprs. cute_fp16/fp8/fp4 family TUs are keyed by dtype only
    (a duplicate explicit instantiation across TUs is a compile error)
    and are macro-gated: CUTE ext for cute_fp16, TMA ext for fp8/fp4. The
    ...
    if cls.enable_tma_ext():
      ...
      yield (
        "fp8_preprocess_instances.cuh",
        cls._render_fp8_preprocess_instances(
          headdims, [t_in for _, t_in, _ in variants]
        ),
      )
      yield ("fwd_fp8_preprocess.cu", cls._render_fp8_preprocess_tu())
    for d in headdims:
  ...
  def _render_fp8_preprocess_instances(cls, headdims, dtypes) -> str:
    ...
    The preprocess TU (fwd_fp8_preprocess.cu) defines FFPA_FP8_
  ...
  def _render_fp8_preprocess_tu(cls) -> str:
    ...
    return (
      ...
      '#include "generated/fp8_preprocess_instances.cuh"\n'
    )
