...
def _attn_bias_grad_reduces_query(
  ...
  """
  return grad_attn_bias is not None and grad_attn_bias.size(
    2
  ) == 1 and seqlen_q > 1
...
def _ffpa_bwd_pre_impl(
  ...
  if D_CHUNK:
    ...
    for d_chunk in range(num_d_chunks):
      ...
      o = tl.load(
        ...
        offs_m[:, None] * stride_om + d_offs[None, :],
        mask=(offs_m[:, None] < seqlen_q) & (d_offs[None, :] < headdim),
        other=0.0,
      ).to(tl.float32)
      do = tl.load(
        DO + off_b * stride_dob + off_h * stride_doh +
        offs_m[:, None] * stride_dom + d_offs[None, :],
        mask=(offs_m[:, None] < seqlen_q) & (d_offs[None, :] < headdim),
        other=0.0,
      ).to(tl.float32)
  ...
  tl.store(
    Delta + off_hb * seqlen_q_rounded + offs_m, delta, mask=offs_m < seqlen_q
  )
...
# global load/add/store accumulation pattern.
@triton.jit
def _ffpa_bwd_dkdv(
  Q: torch.Tensor,
  K: torch.Tensor,
  ...
  if start_n < seqlen_k:
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
      ...
      for d_chunk in range(num_d_chunks):
        ...
        # tl.load/tl.store convert through the pointer element dtype. The
...
# remains a plain store rather than an atomic update.
@triton.jit
def _ffpa_bwd_dq(
  Q: torch.Tensor,
  K: torch.Tensor,
...
@triton.heuristics(_FFPA_BWD_HEURISTICS)
@triton.jit
def _ffpa_bwd_dkdv_kernel_impl(
  Q: torch.Tensor,
  K: torch.Tensor,
  V: torch.Tensor,
  DO: torch.Tensor,
...
@triton.heuristics(_FFPA_BWD_DQ_HEURISTICS)
@triton.jit
def _ffpa_bwd_dq_kernel_impl(
  Q: torch.Tensor,
  K: torch.Tensor,
  V: torch.Tensor,
  DO: torch.Tensor,
...
_ffpa_bwd_dkdv_autotune_cache: dict[tuple[int, str, bool], callable] = {}
_ffpa_bwd_dq_autotune_cache: dict[tuple[int, str], callable] = {}


def _get_bwd_autotune(
  headdim: int, autotune_mode: str, bias_requires_grad: bool
):
...
def _get_bwd_dkdv_autotune(
  headdim: int, autotune_mode: str, bias_requires_grad: bool
):
  """Return an autotune wrapper for the generic split dK/dV kernel."""
  cache_key = (headdim, autotune_mode, bias_requires_grad)
  if cache_key not in _ffpa_bwd_dkdv_autotune_cache:
    reset_args = ["DK", "DV"]
    if bias_requires_grad:
      reset_args.append("GradAttnBias")
    _ffpa_bwd_dkdv_autotune_cache[cache_key] = triton.autotune(
      ...
      reset_to_zero=reset_args,
      cache_results=True,
    )(_ffpa_bwd_dkdv_kernel_impl)
  return _ffpa_bwd_dkdv_autotune_cache[cache_key]


def _get_bwd_dq_autotune(headdim: int, autotune_mode: str):
  """Return an autotune wrapper for the generic split dQ kernel."""
  ...
  if cache_key not in _ffpa_bwd_dq_autotune_cache:
    _ffpa_bwd_dq_autotune_cache[cache_key] = triton.autotune(
      ...
      reset_to_zero=["DQ"],
      cache_results=True,
    )(_ffpa_bwd_dq_kernel_impl)
  return _ffpa_bwd_dq_autotune_cache[cache_key]
...
_ffpa_bwd_decode_stage1_autotune_cache: dict[tuple[int, bool, str, bool],
                                             callable] = {}
  ...
  if USE_GEMV:
    ...
    if BIAS_REQUIRES_GRAD:
      ...
      if GRAD_BIAS_REDUCES_M:
        ...
        grad_bias_ptrs = GradAttnBias + offs_m[:, None] * stride_gbm + offs_n[
          None, :] * stride_gbn
...
def _ffpa_bwd_decode_dq_reduce_kernel(
  ...
  for start_k in range(0, written_k_blocks, BLOCK_K):
    ...
    partial = tl.load(
      PartialDQ + k_blocks[:, None, None] * stride_pqk +
      offs_m[None, :, None] * stride_pqm + offs_d[None, None, :],
      mask=(k_blocks[:, None, None] < written_k_blocks) & mask_m[None, :, None]
...
def _ffpa_attn_backward_triton_impl(
  ...
  * ``dq``, ``dk``, and ``dv`` must already be allocated with the expanded
  ...
  if not autotune and seqlen_q >= 8 and not split_launch:
  ...
  if use_key_bias_grad_reduction:
    min_block_m_for_bias = 64 if autotune or split_launch else int(
      (persisted_main_config or {
        "BLOCK_M": 128
      })["BLOCK_M"]
    )
  ...
  if seqlen_q < 8:
    ...
    written_k_blocks = torch.empty((batch * nheads, ),
                                   dtype=torch.int32,
                                   device=q.device)
  ...
  if split_launch:
    ...
    if autotune:
      _get_bwd_dkdv_autotune(headdim, autotune_mode, main_bias_requires_grad
                             )[dkdv_grid](*dkdv_args, **dkdv_meta)
      _get_bwd_dq_autotune(headdim, autotune_mode)[dq_grid](*dq_args, **dq_meta)
    else:
      dkdv_config = lookup_persistent_config(
        PersistentConfigRequest(
          direction="backward",
...
def _ffpa_attn_backward_triton(
  ...
  * cast the returned gradients back to the original input dtypes
  ...
  :param grad_kv_storage_dtype: Optional storage dtype for the internal Triton
  ...
  """
  if grad_kv_storage_dtype not in (None, torch.float16, torch.float32):
    raise ValueError(
  ...
  if grad_attn_bias.numel() == 0:
    ...
    grad_attn_bias_out = grad_attn_bias.to(
      attn_bias.dtype
    ) if attn_bias is not None else grad_attn_bias
