...
# Adapted from https://github.com/Dao-AILab/flash-attention/blob/main/flash_attn/cute/mma_sm100_desc.py
# Itself a Python port of CUTLASS cute/arch/mma_sm100_desc.hpp and cute/atom/mma_traits_sm100.hpp.
...
def make_instr_desc(
  ...
  """Build the 32-bit Blackwell MMA instruction descriptor; types must be CUTLASS scalar classes."""
...
def mma_op_to_idesc(op: cute.nvgpu.tcgen05.mma.MmaOp):
  return make_instr_desc(
    ...
    op.shape_mnk[0],
    op.shape_mnk[1],
    Major.K if op.a_major_mode == cute.nvgpu.tcgen05.mma.OperandMajorMode.K else
    Major.MN,
    Major.K if op.b_major_mode == cute.nvgpu.tcgen05.mma.OperandMajorMode.K else
    Major.MN,
  )
...
# Figure out the SWIZZLE_* family from the tensor layout.
...
def make_smem_desc_base(
  ...
  """Blackwell 64-bit smem descriptor, minus the start address, for a uint128-tensor layout."""
  ...
  # Strides in units of uint128_t = 16 B.
  ...
  if major is Major.MN:
    ...
    stride_01, stride_11 = canonical_layout.stride[0][
      1], canonical_layout.stride[1][1]
