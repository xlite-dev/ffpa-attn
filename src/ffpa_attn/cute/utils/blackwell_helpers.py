...
# Adapted from https://github.com/Dao-AILab/flash-attention/blob/main/flash_attn/cute/blackwell_helpers.py
...
# TMEM columns one SM100 CTA owns, re-exported from the DSL's arch constant.
SM100_TMEM_CAPACITY_COLUMNS = cute.arch.SM100_TMEM_CAPACITY_COLUMNS
# Per-CTA opt-in SMEM ceiling (227 KiB); each HD512 kernel asserts against it.
SM100_SMEM_CAPACITY_BYTES = cutlass.utils.get_smem_capacity_in_bytes("sm_100")
...
def _tcgen05_mma_kind(op: cute.nvgpu.tcgen05.mma.MmaOp) -> str:
  ...
  if isinstance(op, tcgen05.mma.MmaI8Op):
    return "i8"
  # cutlass-dsl >=4.5.2 builds plain FP8 MMAs as MmaF8F6F4Op, <4.4.x as the legacy MmaFP8Op; both are kind::f8f6f4.
...
def gemm_ptx_partial(
  ...
  if const_expr(not is_ts):
    ...
    smem_desc_base_a: int = const_expr(
      sm100_desc.make_smem_desc_base(
        ...
        sA_swizzle,
        sm100_desc.Major.K if const_expr(
  ...
  tCrA_layout = (
    tCrA.layout if const_expr(not is_ts) else
  ...
  pred_str = "p" if isinstance(
    zero_init, Boolean
  ) else "0" if zero_init else "1"
  if const_expr(not is_ts):
    ...
    # For TS gemm tCrA.iterator.toint() returns 0 whatever the input, so tA_addr has to be passed explicitly.
    tA_addr = tCrA[None, None,
                   0].iterator.toint() if tA_addr is None else tA_addr
    input_args = [
    ...
    llvm.inline_asm(
      ...
      + "".join((
        ...
        1,
        cute.size(tCrA.shape[2]) if const_expr(mbar_ptr is
                                               None) else split_arrive_idx,
      )) + mbar_wait_str + (
        "".join((
          f"add.u32 smem_desc_b_lo, smem_desc_b_lo, {hex(offset_b_diff[k - 1])};\n\t"
          f"mov.b64 smem_desc_b, {{smem_desc_b_lo, smem_desc_b_hi}};\n\t"
        ...
        if const_expr(mbar_ptr is not None) else ""
      ) + "}\n",
