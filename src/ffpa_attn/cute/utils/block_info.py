...
class BlockInfo:
  ...
  def get_n_block_min_max(
    ...
    if const_expr(
      ...
      n_idx_right = n_idx if const_expr(
        self.is_causal
  ...
  @cute.jit
  def get_m_block_min_max(self, seqlen_info: SeqlenInfoQK,
                          n_block: Int32) -> Tuple[Int32, Int32]:
    ...
    if const_expr(
      ...
      m_idx_right = m_idx if const_expr(
        self.is_causal
      ) else m_idx - self.window_size_right
  ...
  def get_n_block_min_causal_local_mask(
    ...
    n_idx = m_idx_min + seqlen_info.seqlen_k - seqlen_info.seqlen_q
    n_idx_right = (
      n_idx if const_expr(not self.is_local or self.window_size_right is None)
      else n_idx + self.window_size_right
    )
