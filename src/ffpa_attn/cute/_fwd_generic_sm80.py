...
class FFPAAttnFwdSm80SplitD:
  ...
  def can_implement(
    ...
    :param dtype: CuTe/CUTLASS element dtype.
    :param head_dim: Query/key head dimension.
    :param head_dim_v: Value head dimension.
    :param tile_m: Query rows per CTA.
    :param tile_n: Key/value rows per CTA.
    :param num_stages: Number of K/V shared-memory pipeline stages.
  ...
  def _setup_attributes(self) -> None:
    ...
    self.sV_layout = cute.tile_to_shape(
      sV_layout_atom, (self.tile_n, self.tile_hdimv, self.num_stages),
      (0, 1, 2)
    )
  ...
  def kernel(
    ...
    if not work_tile.is_valid_tile:
      m_block = cute.ceil_div(mQ.shape[0], self.tile_m)
      head_idx = Int32(0)
      batch_idx = Int32(0)
    ...
    if work_tile.is_valid_tile and n_block_max > 0:
      ...
      for d_group in cutlass.range_constexpr(self.num_d_chunks):
        if const_expr(self.num_stages == 1 or d_group == 0):
          gQ = cute.local_tile(
          ...
          self.load_K(
            gmem_tiled_copy_K.get_slice(tidx),
            gK,
            sK[None, None, 0],
            n_block,
            seqlen.seqlen_k,
        ...
        cute.arch.barrier()
        if const_expr(self.num_stages > 1 and d_group + 1 < self.num_d_chunks):
          gK_next = cute.local_tile(
            mK_cur, (self.tile_n, self.tile_hdim), (n_block, d_group + 1)
          )
          self.load_K(
            gmem_tiled_copy_K.get_slice(tidx),
            gK_next,
            sK[None, None, (d_group + 1) % self.num_stages],
            n_block,
            seqlen.seqlen_k,
          )
          cute.arch.cp_async_commit_group()
        sm80_utils.gemm(
          ...
          tSrK,
          tSsQ,
          tSsK[None, None, None, d_group % self.num_stages],
          smem_thr_copy_Q,
          smem_thr_copy_K,
        )
        cute.arch.barrier()
        if const_expr(self.num_stages > 1 and d_group + 1 < self.num_d_chunks):
          gQ_next = cute.local_tile(
            mQ_cur, (self.tile_m, self.tile_hdim), (m_block, d_group + 1)
          )
          self.load_Q(
            gmem_tiled_copy_Q.get_slice(tidx),
            gQ_next,
            sQ,
            m_block,
            seqlen.seqlen_q,
      ...
      for v_group in cutlass.range_constexpr(self.num_v_chunks):
        ...
        if const_expr(self.num_stages == 1 or v_group == 0):
          gV = cute.local_tile(
            mV_cur, (self.tile_n, self.tile_hdimv), (n_block, v_group)
          )
          self.load_V(
            gmem_tiled_copy_V.get_slice(tidx),
            gV,
            sV[None, None, 0],
            n_block,
            seqlen.seqlen_k,
        ...
        cute.arch.cp_async_wait_group(0)
        self.zero_V_tail(
          sV[None, None, v_group % self.num_stages],
          n_block,
          seqlen.seqlen_k,
          tidx,
        )
        if const_expr(self.num_stages > 1 and v_group + 1 < self.num_v_chunks):
          gV_next = cute.local_tile(
            mV_cur, (self.tile_n, self.tile_hdimv), (n_block, v_group + 1)
          )
          self.load_V(
            gmem_tiled_copy_V.get_slice(tidx),
            gV_next,
            sV[None, None, (v_group + 1) % self.num_stages],
            n_block,
            seqlen.seqlen_k,
          )
          cute.arch.cp_async_commit_group()
        sm80_utils.gemm_rs(
          ...
          tOrP,
          tOrVt,
          tOsVt[None, None, None, v_group % self.num_stages],
          smem_thr_copy_V,
        )
        cute.arch.barrier()
      ...
      for n_iter in cutlass.range(n_block_max - 1, unroll=1):
        ...
        for d_group in cutlass.range_constexpr(self.num_d_chunks):
          ...
          cute.arch.barrier()
          if const_expr(
            self.num_stages > 1 and d_group + 1 < self.num_d_chunks
          ):
            gK_next = cute.local_tile(
              mK_cur, (self.tile_n, self.tile_hdim), (n_block, d_group + 1)
            )
        ...
        for v_group in cutlass.range_constexpr(self.num_v_chunks):
          ...
          if const_expr(
            self.num_stages > 1 and v_group + 1 < self.num_v_chunks
          ):
            gV_next = cute.local_tile(
              mV_cur, (self.tile_n, self.tile_hdimv), (n_block, v_group + 1)
            )
...
class FFPAAttnSm80SplitDPersistQ:
  """SM80/SM89 Split-D forward with persistent full-head-dim Q in shared memory.

  Loads Q once (full ``head_dim``) into a dedicated ``sQ_full`` buffer before
  the key-tile loop and slices it by d-chunk inside the QK MMA loop,
  eliminating redundant global→shared Q traffic.

  Unlike :class:`FFPAAttnFwdSm80SplitD` which aliases ``sQ`` and ``sV`` via
  ``recast_ptr``, this class uses four independent shared-memory buffers
  (``sQ_full``, ``sK``, ``sV``, ``sO``) and therefore requires more SMEM.
  Use :meth:`can_implement` to check whether the target architecture has
  enough shared memory for a given configuration; fall back to
  :class:`FFPAAttnFwdSm80SplitD` when the budget is insufficient.
  """
  ...
  def can_implement(
    ...
    """Check whether PersistQ configuration fits in shared memory.

    Four independent buffers: ``sQ_full`` (full head_dim), ``sK``, ``sV``,
    and ``sO`` each occupy separate SMEM regions.
    """
    del is_causal
    if head_dim_v is None:
      head_dim_v = head_dim
    ...
    smem_usage_K = tile_n * d_chunk * num_stages * 2
    smem_usage_V = tile_n * d_chunk * num_stages * 2
    smem_usage_O = tile_m * d_chunk * 2
    smem_usage = smem_usage_Q + smem_usage_K + smem_usage_V + smem_usage_O
    smem_capacity = utils_basic.get_smem_capacity_in_bytes(smem_capacity_arch)
    return smem_usage <= smem_capacity

  # _check_type is identical to FFPAAttnFwdSm80SplitD.
  def _check_type(
    self,
    mQ_type: Type[cutlass.Numeric],
    mK_type: Type[cutlass.Numeric],
  ...
  def _setup_attributes(self) -> None:
    ...
    self.sQ_full_layout = cute.tile_to_shape(
      sQ_layout_atom, (self.tile_m, self.head_dim), (0, 1)
    )
    self.sK_layout = cute.tile_to_shape(
      sK_layout_atom, (self.tile_n, self.tile_hdim, self.num_stages), (0, 1, 2)
    )
  ...
  # _get_tiled_mma is identical to FFPAAttnFwdSm80SplitD.
  def _get_tiled_mma(self):
    tiled_mma_qk = cute.make_tiled_mma(
      warp.MmaF16BF16Op(self.dtype, Float32, (16, 8, 16)),
      (self.num_threads // 32, 1, 1),
  ...
  def _get_shared_storage_cls(self):
    sQ_full_struct = cute.struct.Align[cute.struct.MemRange[
      self.dtype, cute.cosize(self.sQ_full_layout)], 1024]
    sK_struct = cute.struct.Align[cute.struct.MemRange[
      self.dtype, cute.cosize(self.sK_layout)], 1024]
    sV_struct = cute.struct.Align[cute.struct.MemRange[
      self.dtype, cute.cosize(self.sV_layout)], 1024]
    sO_struct = cute.struct.Align[cute.struct.MemRange[
      self.dtype, cute.cosize(self.sO_layout)], 1024]

    @cute.struct
    class SharedStorage:
      sQ_full: sQ_full_struct  # type: ignore
      sK: sK_struct  # type: ignore
      sV: sV_struct  # type: ignore
      sO: sO_struct  # type: ignore
  ...
  def __call__(
    ...
    """Configure and launch the SM80 Split-D PersistQ forward kernel."""
    self._check_type(
      *(
        t.element_type if t is not None else None
        for t in (mQ, mK, mV, mO, mLSE, mCuSeqlensQ, mCuSeqlensK)
  ...
  def kernel(
    ...
    softmax_scale_log2: Float32,
    softmax_scale: Optional[Float32],
    sQ_full_layout: cute.ComposedLayout,
    sK_layout: cute.ComposedLayout,
    sV_layout: cute.ComposedLayout,
    sO_layout: cute.ComposedLayout,
    gmem_tiled_copy_Q: cute.TiledCopy,
    ...
    smem = cutlass.utils.SmemAllocator()
    storage = smem.allocate(SharedStorage)
    sQ_full = storage.sQ_full.get_tensor(sQ_full_layout)
    sK = storage.sK.get_tensor(sK_layout)
    ...
    thr_mma_qk = tiled_mma_qk.get_slice(tidx)
    thr_mma_pv = tiled_mma_pv.get_slice(tidx)
    sQ_d0 = cute.local_tile(sQ_full, (self.tile_m, self.tile_hdim), (0, 0))
    tSrQ = thr_mma_qk.make_fragment_A(thr_mma_qk.partition_A(sQ_d0))
    tSrK = thr_mma_qk.make_fragment_B(thr_mma_qk.partition_B(sK[None, None, 0]))
    tOrVt = thr_mma_pv.make_fragment_B(
      thr_mma_pv.partition_B(sVt[None, None, 0])
    )
    ...
    if work_tile.is_valid_tile and n_block_max > 0:
      # Prologue: load full Q once into persistent sQ_full.
      gQ_full = cute.local_tile(
        mQ_cur, (self.tile_m, self.head_dim), (m_block, 0)
      )
      self.load_Q_full(
        gmem_tiled_copy_Q.get_slice(tidx),
        gQ_full,
      ...
      # Peel first KV tile (n_block=0, is_first=True).
      n_block = Int32(0)
      acc_shape_S = thr_mma_qk.partition_shape_C((self.tile_m, self.tile_n))
      acc_S = cute.make_rmem_tensor(acc_shape_S, Float32)
      acc_S.fill(0.0)
      for d_group in cutlass.range_constexpr(self.num_d_chunks):
        ...
        if const_expr(self.num_stages > 1 and d_group + 1 < self.num_d_chunks):
          ...
          cute.arch.cp_async_commit_group()
        sQ_d = cute.local_tile(
          sQ_full, (self.tile_m, self.tile_hdim), (0, d_group)
        )
        tSsQ_d = smem_thr_copy_Q.partition_S(sQ_d)
        sm80_utils.gemm(
          thr_mma_qk,
          acc_S,
          tSrQ,
  ...
  # apply_mask is identical to FFPAAttnFwdSm80SplitD.
  @cute.jit
  def apply_mask(
    self,
    acc_S: cute.Tensor,
  ...
  def load_Q_full(
    ...
    """Load full head_dim Q tile from global to persistent shared memory."""
    tQsQ, tQgQ = gmem_thr_copy.partition_D(sQ_full
                                           ), gmem_thr_copy.partition_S(gQ)
    cQ = cute.make_identity_tensor((self.tile_m, self.head_dim))
    tQcQ = gmem_thr_copy.partition_S(cQ)
    t0QcQ = gmem_thr_copy.get_slice(0).partition_S(cQ)
    for m in cutlass.range_constexpr(cute.size(tQsQ.shape[1])):
      if t0QcQ[0, m, 0][0] < seqlen - block * self.tile_m - tQcQ[0][0]:
  ...
  # load_K, load_V, zero_V_tail, store_O_and_lse are identical to
  # FFPAAttnFwdSm80SplitD — included inline for self-containedness.
  @cute.jit
  def load_K(
    self,
    gmem_thr_copy: cute.TiledCopy,
...
__all__ = ["FFPAAttnFwdSm80SplitD", "FFPAAttnSm80SplitDPersistQ"]
