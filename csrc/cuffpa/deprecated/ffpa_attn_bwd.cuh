...
//       entry, stream Q/dO/O by D-slice, compute S/dP in Phase 1, then reuse
//       the resident K in Phase 2 for dQ.  Q/dO still need to be streamed for
//       dK/dV, and dQ plus dK/dV may still use atomicAdd initially.  The main
//       expected win is removing repeated global K/V loads across Q tiles and
//       avoiding the Phase-2 K reload when SMEM budget allows it.
//
// ----------------------------------------------------------------------------
...
__device__ __forceinline__ int c_frag_local_row(const int lane_id,
                                                const int reg_id) {
  return (lane_id / 4) + ((reg_id >= 2) ? 8 : 0);
}

__device__ __forceinline__ int c_frag_local_col(const int lane_id,
                                                const int n_tile_id,
                                                const int reg_id) {
...
__device__ __forceinline__ void compute_p_and_ds_from_lse(
    ...
    uint32_t (&R_P)[kNumNTiles][2], uint32_t (&R_dS)[kNumNTiles][2],
    const float lse_row0, const float lse_row8, const float dp_sum_row0,
    const float dp_sum_row8, const float scale) {
...
__device__ __forceinline__ void atomic_add_c_frag_to_dq(
    const uint32_t (&R_C)[4], kDataType* __restrict__ dQ,
    const int q_gmem_offset, const int Q_tile_id, const int warp_QP,
    const int d_col_base, const int Nq, const float scale) {
  using Traits = DtypeTraits<kDataType>;
...
#pragma unroll
  for (int reg_id = 0; reg_id < 4; ++reg_id) {
    const int row =
        Q_tile_id * 64 + warp_QP * 16 + c_frag_local_row(lane_id, reg_id);
...
__device__ __forceinline__ void store_packed_c_frag_transposed_to_smem(
  ...
  static_assert(sizeof(kDataType) == 2,
                "packed C-fragment store expects 16-bit activation dtype.");
  const int lane_id = threadIdx.x % WARP_SIZE;
  ...
  for (int n_tile = 0; n_tile < kNumNTiles; ++n_tile) {
    ...
    if (store_row_pair) {
      ...
      const uint32_t packed0 =
          (frag01 & 0x0000ffffu) | ((next_frag01 & 0x0000ffffu) << 16);
      const uint32_t packed1 =
          ((frag01 & 0xffff0000u) >> 16) | (next_frag01 & 0xffff0000u);
      const uint32_t packed2 =
          (frag23 & 0x0000ffffu) | ((next_frag23 & 0x0000ffffu) << 16);
      const uint32_t packed3 =
          ((frag23 & 0xffff0000u) >> 16) | (next_frag23 & 0xffff0000u);
      *reinterpret_cast<uint32_t*>(&smem_tile[col0 * kSmemStride + row0]) =
          packed0;
      *reinterpret_cast<uint32_t*>(&smem_tile[col1 * kSmemStride + row0]) =
          packed1;
      *reinterpret_cast<uint32_t*>(&smem_tile[col0 * kSmemStride + row8]) =
          packed2;
      *reinterpret_cast<uint32_t*>(&smem_tile[col1 * kSmemStride + row8]) =
          packed3;
    }
  }
}
...
template <typename kDataType, const int kHeadDim>
__device__ __forceinline__ void atomic_add_c_frag_to_dkv(
    const uint32_t (&R_C)[4], kDataType* __restrict__ dKV,
    const int kv_gmem_offset, const int K_tile_id, const int kv_frag,
    const int d_col_base, const int Nkv, const float scale) {
  using Traits = DtypeTraits<kDataType>;
  const int lane_id = threadIdx.x % WARP_SIZE;
  const float* c_frag = reinterpret_cast<const float*>(&R_C[0]);
#pragma unroll
  for (int reg_id = 0; reg_id < 4; ++reg_id) {
    const int row =
        K_tile_id * 64 + kv_frag * 16 + c_frag_local_row(lane_id, reg_id);
    const int col = d_col_base + (lane_id % 4) * 2 + (reg_id & 1);
...
template <typename kDataType, const int kHeadDim>
__device__ __forceinline__ float dot_do_o_row_4lane(
    const kDataType* __restrict__ dO, const kDataType* __restrict__ O,
    const int q_gmem_offset, const int row, const int Nq) {
  using Traits = DtypeTraits<kDataType>;
  const int lane_id = threadIdx.x % WARP_SIZE;
  const int lane_col = lane_id % 4;
  float acc = 0.0f;
...
template <typename kDataType, const int Br, const int Bc, const int QTileSize,
          const int KTileSize, const int VTileSize, const int kHeadDim,
...
__device__ __forceinline__ void cp_async_phase1_g2s(
    ...
    const uint32_t smem_K_base_ptr, const uint32_t smem_V_base_ptr,
    const kDataType* __restrict__ Q, const kDataType* __restrict__ dO,
    const kDataType* __restrict__ K, const kDataType* __restrict__ V,
    const int q_gmem_offset, const int kv_gmem_offset, const int Q_tile_id,
    const int K_tile_id, const int tile_d, const int stage, const int Nq,
    const int Nkv) {
  ffpa::prefill::cp_async_qkv_g2s<Br, QTileSize, kHeadDim, kMmaAtomK,
                                  kNumThreads, kPadQ>(
      smem_Q_base_ptr, Q, q_gmem_offset, Q_tile_id, tile_d, stage, Nq);
  ffpa::prefill::cp_async_qkv_g2s<Br, QTileSize, kHeadDim, kMmaAtomK,
                                  kNumThreads, kPadQ>(
      smem_dO_base_ptr, dO, q_gmem_offset, Q_tile_id, tile_d, stage, Nq);
  ffpa::prefill::cp_async_qkv_g2s<Bc, KTileSize, kHeadDim, kMmaAtomK,
                                  kNumThreads, kPadK>(
      smem_K_base_ptr, K, kv_gmem_offset, K_tile_id, tile_d, stage, Nkv);
  ffpa::prefill::cp_async_qkv_g2s<Bc, VTileSize, kHeadDim, kMmaAtomK,
                                  kNumThreads, kPadV>(
...
template <typename kDataType, const int Br, const int Bc, const int QTileSize,
          const int KTileSize, const int kHeadDim, const int kMmaAtomK,
          const int kNumThreads, const int kPadQ, const int kPadK>
__device__ __forceinline__ void cp_async_phase2_g2s(
    const uint32_t smem_Q_base_ptr, const uint32_t smem_dO_base_ptr,
    const uint32_t smem_K_base_ptr, const kDataType* __restrict__ Q,
...
__global__ void __launch_bounds__(WARP_SIZE* kMmaTileSeqLenQ* kMmaTileSeqLenK)
    ffpa_attn_split_d_bwd_template(
        ...
        const kDataType* __restrict__ dO, const kDataType* __restrict__ O,
        kDataType* __restrict__ dQ, kDataType* __restrict__ dK,
        kDataType* __restrict__ dV, const int Nq, const int Nkv, const int Nh,
        const int Nh_kv, const float scale, const int Tc, const int causal) {
  ...
  static_assert(Br == 64,
                "Split-D backward stage-1 target currently requires Br=64.");
  static_assert(Bc == 64,
                "Split-D backward stage-1 target currently requires Bc=64.");
...
#pragma unroll 1
    for (int Q_tile_id = Tr - 1; Q_tile_id >= 0; --Q_tile_id) {
      ...
      // precompute D[row] once per (B,H,Q) row, as FlashAttention backward
      // does, or fuse it into a Q-driven prepass to remove this repeated global
      // IO.
      const float dp_sum_row0 =
          ffpa::bwd::dot_do_o_row_4lane<kDataType, kHeadDim>(
              dO, O, q_gmem_offset, q_row0, Nq);
      const float dp_sum_row8 =
          ffpa::bwd::dot_do_o_row_4lane<kDataType, kHeadDim>(
              dO, O, q_gmem_offset, q_row8, Nq);
...
#pragma unroll
        for (int stage = 0; stage < (kStageQK - 1); ++stage) {
          ffpa::bwd::cp_async_phase1_g2s<
              kDataType, Br, Bc, Q_tile_size, K_tile_size, V_tile_size,
              kHeadDim, kMmaAtomK, kNumThreads, kPadQ, kPadK, kPadV>(
              smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr,
              smem_V_base_ptr, Q, dO, K, V, q_gmem_offset, kv_gmem_offset,
              Q_tile_id, K_tile_id, stage, stage, Nq, Nkv);
        }
...
#pragma unroll 1
      for (int tile_d = 0; tile_d < (kHeadDim / kMmaAtomK); ++tile_d) {
        ...
        if constexpr (kStageQK > 1) {
          ...
          if (next_tile_d < (kHeadDim / kMmaAtomK)) {
            ffpa::bwd::cp_async_phase1_g2s<
                ...
                smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr,
                smem_V_base_ptr, Q, dO, K, V, q_gmem_offset, kv_gmem_offset,
                Q_tile_id, K_tile_id, next_tile_d, smem_sel_next, Nq, Nkv);
          }
        } else {
          ffpa::bwd::cp_async_phase1_g2s<
              kDataType, Br, Bc, Q_tile_size, K_tile_size, V_tile_size,
              ...
              smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr,
              smem_V_base_ptr, Q, dO, K, V, q_gmem_offset, kv_gmem_offset,
              Q_tile_id, K_tile_id, tile_d, smem_sel, Nq, Nkv);
        ...
        ffpa::prefill::sync_fetch_qkv_frags_s2r<0, 4, Q_tile_size, kMmaAtomM,
                                                ...
                                                kDataType>(
            smem_Q_base_ptr, &R_Q[0], warp_QP, 0, 0, smem_sel);
        ffpa::prefill::sync_fetch_qkv_frags_s2r<0, 4, Q_tile_size, kMmaAtomM,
                                                kMmaAtomN, kMmaAtomK, kPadQ,
                                                kDataType>(
            smem_dO_base_ptr, &R_dO[0], warp_QP, 0, 0, smem_sel);
...
#pragma unroll
        for (int kv_frag = 0; kv_frag < kValTileSeqLenK; ++kv_frag) {
          ...
          ffpa::prefill::sync_fetch_qkv_frags_s2r<0, 2, V_tile_size, kMmaAtomM,
          ...
          ffpa::mma::m16n8k16_abf32<kDataType,
              ...
              &R_S[kv_frag][0], &R_S[kv_frag][1], &R_S[kv_frag][2],
              &R_S[kv_frag][3], &R_Q[0], &R_Q[1], &R_Q[2], &R_Q[3], &R_K[0],
              &R_K[1]);
          ffpa::mma::m16n8k16_abf32<kDataType,
                                    ffpa::mma::MMAMode::kInplaceUpdate>(
              &R_dP[kv_frag][0], &R_dP[kv_frag][1], &R_dP[kv_frag][2],
              &R_dP[kv_frag][3], &R_dO[0], &R_dO[1], &R_dO[2], &R_dO[3],
              &R_VK[0], &R_VK[1]);
        }
        if constexpr (kStageQK > 1) {
      ...
      const float lse_row0 =
          (q_row0 < Nq) ? softmax_lse[lse_offset + q_row0] : INFINITY;
      const float lse_row8 =
          (q_row8 < Nq) ? softmax_lse[lse_offset + q_row8] : INFINITY;
      ...
      ffpa::bwd::compute_p_and_ds_from_lse<kDataType, kValTileSeqLenK>(
          R_S, R_dP, R_P, R_dS, lse_row0, lse_row8, dp_sum_row0, dp_sum_row8,
          scale);
      ...
      // to shared memory and reloads it with ldmatrix.x4. This extra smem
      // write, read, and barrier are expensive. A high-value future
      // optimization is a register-level C-fragment -> A-fragment transpose
      // that removes this scratch path entirely.
      ...
      ffpa::bwd::store_packed_c_frag_transposed_to_smem<
          kDataType, kValTileSeqLenK, kMmaAtomK + kTransposeScratchPad>(
          R_P, warp_P_t_smem);
      ffpa::bwd::store_packed_c_frag_transposed_to_smem<
          kDataType, kValTileSeqLenK, kMmaAtomK + kTransposeScratchPad>(
          R_dS, warp_dS_t_smem);
      __syncthreads();
      const uint32_t smem_warp_P_t_base_ptr =
          __cvta_generic_to_shared(warp_P_t_smem);
      const uint32_t smem_warp_dS_t_base_ptr =
          __cvta_generic_to_shared(warp_dS_t_smem);
      ...
      // D-slice and write each KV tile once, avoiding global atomics while
      // still staying within the per-block shared-memory budget.
      if constexpr (kStageQK > 1) {
#pragma unroll
        for (int stage = 0; stage < (kStageQK - 1); ++stage) {
          ffpa::bwd::cp_async_phase2_g2s<kDataType, Br, Bc, Q_tile_size,
                                         K_tile_size, kHeadDim, kMmaAtomK,
                                         kNumThreads, kPadQ, kPadK>(
              smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr, Q, dO, K,
              q_gmem_offset, kv_gmem_offset, Q_tile_id, K_tile_id, stage, stage,
              Nq, Nkv);
        }
        ...
        if constexpr (kStageQK > 1) {
          ...
          if (next_tile_d < (kHeadDim / kMmaAtomK)) {
            ffpa::bwd::cp_async_phase2_g2s<kDataType, Br, Bc, Q_tile_size,
                                           ...
                                           kNumThreads, kPadQ, kPadK>(
                smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr, Q, dO, K,
                q_gmem_offset, kv_gmem_offset, Q_tile_id, K_tile_id,
                next_tile_d, smem_sel_next, Nq, Nkv);
          }
        } else {
          ffpa::bwd::cp_async_phase2_g2s<kDataType, Br, Bc, Q_tile_size,
                                         K_tile_size, kHeadDim, kMmaAtomK,
                                         kNumThreads, kPadQ, kPadK>(
              smem_Q_base_ptr, smem_dO_base_ptr, smem_K_base_ptr, Q, dO, K,
              q_gmem_offset, kv_gmem_offset, Q_tile_id, K_tile_id, tile_d,
              smem_sel, Nq, Nkv);
          ffpa::cp_async::wait_group<0>();
          __syncthreads();
        }
...
#pragma unroll
          for (int kv_frag = 0; kv_frag < (Bc / kMmaAtomK); ++kv_frag) {
            ffpa::prefill::sync_fetch_qkv_frags_s2r<
                1, 2, K_tile_size, kMmaAtomM, kMmaAtomN, kMmaAtomK, kPadK,
                kDataType>(smem_K_base_ptr, &R_K[0], warp_KV, d_subtile,
                           kv_frag, smem_sel);
            const int ds_offset = kv_frag * 2;
            ffpa::mma::m16n8k16_abf32<kDataType,
                                      ffpa::mma::MMAMode::kInplaceUpdate>(
                &R_dQ[0], &R_dQ[1], &R_dQ[2], &R_dQ[3], &R_dS[ds_offset][0],
                &R_dS[ds_offset][1], &R_dS[ds_offset + 1][0],
                &R_dS[ds_offset + 1][1], &R_K[0], &R_K[1]);
          }
          const int d_col_base = tile_d * kMmaAtomK + d_subtile * kMmaAtomN;
          ffpa::bwd::atomic_add_c_frag_to_dq<kDataType, kHeadDim>(
              R_dQ, dQ, q_gmem_offset, Q_tile_id, warp_QP, d_col_base, Nq,
              scale);

          ffpa::prefill::sync_fetch_qkv_frags_s2r<1, 2, Q_tile_size, kMmaAtomM,
                                                  kMmaAtomN, kMmaAtomK, kPadQ,
                                                  kDataType>(
          ...
          ffpa::prefill::sync_fetch_qkv_frags_s2r<1, 2, Q_tile_size, kMmaAtomM,
                                                  kMmaAtomN, kMmaAtomK, kPadQ,
                                                  kDataType>(
              smem_dO_base_ptr, &R_dOB[0], warp_KV, d_subtile, warp_QP,
              smem_sel);
#pragma unroll
          for (int kv_frag = 0; kv_frag < (Bc / kMmaAtomK); ++kv_frag) {
            ...
            ffpa::prefill::sync_fetch_qkv_frags_s2r<
                0, 4, Transpose_tile_size, kMmaAtomM, kMmaAtomN, kMmaAtomK,
                kTransposeScratchPad, kDataType>(smem_warp_dS_t_base_ptr,
                                                 &R_dS_T[0], kv_frag, 0, 0, 0);
            ffpa::prefill::sync_fetch_qkv_frags_s2r<
                0, 4, Transpose_tile_size, kMmaAtomM, kMmaAtomN, kMmaAtomK,
                kTransposeScratchPad, kDataType>(smem_warp_P_t_base_ptr,
                                                 &R_P_T[0], kv_frag, 0, 0, 0);
            ffpa::mma::m16n8k16_abf32<kDataType,
                                      ffpa::mma::MMAMode::kInplaceUpdate>(
                &R_dK[0], &R_dK[1], &R_dK[2], &R_dK[3], &R_dS_T[0], &R_dS_T[1],
                &R_dS_T[2], &R_dS_T[3], &R_QB[0], &R_QB[1]);
            ffpa::mma::m16n8k16_abf32<kDataType,
                                      ffpa::mma::MMAMode::kInplaceUpdate>(
                &R_dV[0], &R_dV[1], &R_dV[2], &R_dV[3], &R_P_T[0], &R_P_T[1],
                &R_P_T[2], &R_P_T[3], &R_dOB[0], &R_dOB[1]);
            ffpa::bwd::atomic_add_c_frag_to_dkv<kDataType, kHeadDim>(
                R_dK, dK, kv_gmem_offset, K_tile_id, kv_frag, d_col_base, Nkv,
                scale);
            ffpa::bwd::atomic_add_c_frag_to_dkv<kDataType, kHeadDim>(
                R_dV, dV, kv_gmem_offset, K_tile_id, kv_frag, d_col_base, Nkv,
                1.0f);
          }
        }
        if constexpr (kStageQK > 1) {
...
// ffpa_attn_persistent_kv_bwd_template and its helpers have been removed.
// See commit history or git-blame for the removed code.
// The Triton backward kernel (src/ffpa_attn/triton/_ffpa_bwd.py) is now the
// primary development focus for FFPA backward optimization.
