#pragma once
// CuTe fp16/bf16 split_d launcher: kernel include + bias plan + variant
...
// kernel. Mirrors the pre-split inline plan in cute_fp16.cuh; keep in
// sync with the kernel's kBiasStages and the 99KB opt-in budget.
template <typename kDataType, const int kHeadDim, const int kStage,
          const int kQKDChunk, const int kVDChunk>
inline FfpaBiasTilePlan fp16_split_d_bias_plan(const FfpaBiasParams& bias_p,
  ...
  if (kBaseSmemBytes + plan.tile_bytes(kBr, kBc, bias_stages) >
      kSmemBudgetBytes)
    plan.mode = 0;
  // Row-broadcast resident upgrade (mode 3): keep the smem-driven
  // occupancy of the base layout (101376B opt-in per block).
  if (plan.mode == 2) {
...
// Split-D (non-WS) M8N1 launcher, %64 headdims use (32,64) chunks and
// %32-not-%64 use (32,32). Variant tags: kBiasOn (attn_bias tensor
// present), kBiasPlanMode (bias tile mode: 0 = gmem-direct fallback,
// 1 = dense [kBr,kBc] TMA tile, 2 = row-broadcast TMA, 3 = resident row
// vector), kBias4BytesPerElem (1 = 4-byte fp32 mask, 0 = 2-byte
// fp16/bf16 mask), kHasDropout (dropout plumbing). The umbrella wrapper
// computes the plan and picks the exact tag; explicit instantiations
// live in the generated variant TUs.
template <typename kDataType, const int kHeadDim, const int kStage,
          const int kQKDChunk, const int kVDChunk, int kBiasOn,
          int kBiasPlanMode, int kBias4BytesPerElem, int kHasDropout>
void launch_cute_fwd_split_d_sm120_v(torch::Tensor Q, torch::Tensor K,
  ...
  if (bias_on) {
    bias_plan =
        ffpa::fp16_split_d_bias_plan<kDataType, kHeadDim, kStage, kQKDChunk,
                                     kVDChunk>(bias, Nb, Nh, Nq, Nkv);
  }
  TORCH_CHECK(kBiasOn == bias_on &&
              ...
              "ffpa_attn: fp16 split_d D=", kHeadDim,
  ...
  bool dropout_bitmap_on =
      kHasDropout != 0 && getenv("FFPA_DROPOUT_BITMAP_DISABLE") == nullptr;
#else
  bool dropout_bitmap_on = kHasDropout != 0;
#endif
  ...
  const auto run = [&](auto tma_q, auto tma_k, auto tma_v, auto q_c,
    ...
    if constexpr (kBiasOn == 0) {
      launch_variant(split_d_fwd_cute_sm120<Traits, TmaQ, TmaK, TmaV, TmaO,
                                            decltype(tma_bias_r16), 0, 0, 0,
                                            kHasDropout, kNhdQ, kNhdKV>,
                     tma_bias_r16);
    } else if constexpr (kBiasPlanMode == 1) {
      if constexpr (kBias4BytesPerElem)
        launch_variant(split_d_fwd_cute_sm120<Traits, TmaQ, TmaK, TmaV, TmaO,
                                              decltype(tma_bias_d32),
                                              kBiasPlanMode, kBias4BytesPerElem,
                                              1, kHasDropout, kNhdQ, kNhdKV>,
                       tma_bias_d32);
      else
        launch_variant(split_d_fwd_cute_sm120<Traits, TmaQ, TmaK, TmaV, TmaO,
                                              decltype(tma_bias_d16),
                                              kBiasPlanMode, kBias4BytesPerElem,
                                              1, kHasDropout, kNhdQ, kNhdKV>,
                       tma_bias_d16);
      ...
      if constexpr (kBias4BytesPerElem)
        launch_variant(split_d_fwd_cute_sm120<Traits, TmaQ, TmaK, TmaV, TmaO,
                                              decltype(tma_bias_r32),
                                              kBiasPlanMode, kBias4BytesPerElem,
                                              1, kHasDropout, kNhdQ, kNhdKV>,
                       tma_bias_r32);
      else
        launch_variant(split_d_fwd_cute_sm120<Traits, TmaQ, TmaK, TmaV, TmaO,
                                              decltype(tma_bias_r16),
                                              kBiasPlanMode, kBias4BytesPerElem,
                                              1, kHasDropout, kNhdQ, kNhdKV>,
                       tma_bias_r16);
