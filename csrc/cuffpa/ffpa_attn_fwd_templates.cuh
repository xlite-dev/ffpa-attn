...
__global__ void __launch_bounds__(256) ffpa_attn_splitkv_decode_stage1_template(
  ...
  constexpr int kColsPerThread = (kHeadDim + kNumThreads - 1) / kNumThreads;
...
#pragma unroll
  ...
  for (int row = 0; row < active_rows; ++row) {
    ...
    // Phase 1: scatter acc registers → smem_out.
#pragma unroll
    for (int col = 0; col < kColsPerThread; ++col) {
      const int d = tid + col * kNumThreads;
      if (d < kHeadDim) {
        smem_out[d] = acc[row][col] * inv_l;
      }
    }
    __syncthreads();
...
__global__ void __launch_bounds__(256)
  ...
  // kNumVecF32 = kHeadDim / 8 (e.g. D=512 → 64), kNumThreads=256,
  // so tid 0..63 each handles one 8×fp32 vector per split loop.
  ...
  // Vectorized 128-bit O store.
  // kHeadDim/kVecO = D/8 for half (e.g. D=512 → 64 vectors), kNumThreads=256,
  // so tid 0..63 each stores one 8×half vector, covering all D elements.
