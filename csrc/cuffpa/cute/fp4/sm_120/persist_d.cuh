...
struct Fp4PersistDTraits {
  ...
  static_assert(kSmemBytes <= 99 * 1024, "smem budget");
...
// NVFP4 persist-D forward. Grid: (ceil(Nq/kBr), Nb*Nh). Workspaces are
// 128-padded along seqlen; TMA descriptors are built on the padded flat row
// spaces (Q/K/V^T) and on the SF atom-layout tensors (SFQ/SFK/SFVt) and the
// (B,H,Mb,Nkv_pad) delta_s tensor (DS). lse (natural log, with the smooth-K
// correction) is written when softmax_lse != nullptr; km/qm may be null to
// skip the correction.
...
__global__ void __launch_bounds__(384, 1) persist_d_ws_fwd_cute_fp4_sm120(
  ...
  const int q_row_offset = q_bh * Nq_pad + q_start_row;  // padded Q row space
  const int kv_row_offset = kv_bh * Nkv_pad;             // padded K row space
  const int v_row_base = kv_bh * kHeadDim;               // V^T D-plane rows
  const int O_row_offset = q_bh * Nq + q_start_row;      // true O row space
  const int Nb = gridDim.y / Nh;
  ...
  if (is_producer) {
    ...
    if (wg_tid == 0) {
      // Flat descriptor spaces mirror the launcher: Q/K are
      // (Nb*H*_pad, D) row planes, V^T is (Nb*Hkv*D, Nkv_pad).
      auto mQ = domain_offset(make_coord(q_row_offset, 0),
                              tma_q.get_tma_tensor(make_shape(
                                  (long)Nb * Nh * Nq_pad, Int<kHeadDim>{})));
      auto mK =
          domain_offset(make_coord(kv_row_offset, 0),
                        tma_k.get_tma_tensor(make_shape(
                            (long)Nb * Nh_kv * Nkv_pad, Int<kHeadDim>{})));
      auto mV = domain_offset(make_coord(v_row_base, _0{}),
                              tma_v.get_tma_tensor(make_shape(
                                  (long)Nb * Nh_kv * kHeadDim, Nkv_pad)));
      ...
      auto gK =
          local_tile(mK, Shape<Int<kBc>, Int<kHeadDim>>{}, make_coord(_, _0{}));
      auto gV =
          local_tile(mV, Shape<Int<kHeadDim>, Int<kBc>>{}, make_coord(_0{}, _));
      auto gSFQ =
          local_tile(mSFQ(_, _, Nh_id, Nb_id), Shape<Int<kBr>, Int<kHeadDim>>{},
                     make_coord(q_tile_abs, _0{}));
      auto gSFK =
          local_tile(mSFK(_, _, kv_head_idx, Nb_id),
                     Shape<Int<kBc>, Int<kHeadDim>>{}, make_coord(_, _0{}));
      auto gSFVt =
          local_tile(mSFVt(_, _, kv_head_idx, Nb_id),
                     Shape<Int<kHeadDim>, Int<kBc>>{}, make_coord(_0{}, _));
      auto gDS =
          local_tile(mDS(_, _, Nh_id, Nb_id), Shape<Int<kBr>, Int<kBc>>{},
      ...
      for (int s = 0; s < kStages - 1; ++s) {
        if (s < Tc_eff) {
          ...
          TmaBarrier::arrive_and_expect_tx(&k_full[s], Traits::kTxBytesK);
          copy(tma_k.with(k_full[s]), tKgK(_, s), tKsK(_, s));
          copy(tma_sfk.with(k_full[s]), tKgSFK(_, s), tKsSFK(_, s));
          copy(tma_ds.with(k_full[s]), tDSgDS(_, s), tDSsDS(_, s));
        }
      }
      for (int s = 0; s < kStages - 1; ++s) {
        if (s < Tc_eff) {
          CtaBarrier::wait(&v_empty[s], 0);
          TmaBarrier::arrive_and_expect_tx(&v_full[s], Traits::kTxBytesV);
          copy(tma_v.with(v_full[s]), tVgV(_, s), tVsV(_, s));
          copy(tma_sfvt.with(v_full[s]), tVgSFVt(_, s), tVsSFVt(_, s));
        }
      }
      for (int tile = 0; tile < Tc_eff; ++tile) {
        {
          ...
          if (v_tile < Tc_eff) {
            const int stage_v = v_tile % kStages;
            const int phase_v = (v_tile / kStages) & 1;
            CtaBarrier::wait(&v_empty[stage_v], phase_v);
            TmaBarrier::arrive_and_expect_tx(&v_full[stage_v],
                                             Traits::kTxBytesV);
            copy(tma_v.with(v_full[stage_v]), tVgV(_, v_tile),
                 tVsV(_, stage_v));
            copy(tma_sfvt.with(v_full[stage_v]), tVgSFVt(_, v_tile),
                 tVsSFVt(_, stage_v));
          }
        }
        {
          ...
          if (k_tile < Tc_eff) {
            const int stage_k = k_tile % kStages;
            const int phase_k = (k_tile / kStages) & 1;
            CtaBarrier::wait(&k_empty[stage_k], phase_k);
            TmaBarrier::arrive_and_expect_tx(&k_full[stage_k],
                                             Traits::kTxBytesK);
            copy(tma_k.with(k_full[stage_k]), tKgK(_, k_tile),
                 tKsK(_, stage_k));
            copy(tma_sfk.with(k_full[stage_k]), tKgSFK(_, k_tile),
                 tKsSFK(_, stage_k));
            copy(tma_ds.with(k_full[stage_k]), tDSgDS(_, k_tile),
                 tDSsDS(_, stage_k));
          }
        }
      }
...
#pragma unroll 1
  for (int kv_tile = 0; kv_tile < Tc_eff; ++kv_tile) {
    ...
    const int k_phase = (kv_tile / kStages) & 1;
    const int v_stg = kv_tile % kStages;
    const int v_phase = (kv_tile / kStages) & 1;
    ...
    // valid columns all land outside this tile.
    {
      ...
      if (tail_tile || causal_tile) {
        ...
        for (int row = 0; row < kSRows; ++row) {
          const int q_pos =
              q_start_row + Br_base + cute::get<0>(tScS_rc(row, 0)) + kv_offset;
          CUTLASS_PRAGMA_UNROLL
