...
void launch_cute_fwd_split_d_sm120(torch::Tensor Q, torch::Tensor K,
  ...
  auto tma_bias_r32 = make_tma_bias(std::integral_constant<int, 2>{},
                                    std::integral_constant<int, 1>{});
  int kSmemBytes = ((kBaseSmemBytes + 15) & ~15) +
...
void launch_cute_fwd_persist_d_sm120(torch::Tensor Q, torch::Tensor K,
  ...
  // (inside the K/V TMA wait window). The env turns it off (A/B + escape
  // hatch -> inline per-element Philox) and is read per call so toggling
  // mid-process works. kBc=32 (D=192/256) can't use the half-row scheme
  // (a row must span an even number of 32-bit words) and stays inline.
