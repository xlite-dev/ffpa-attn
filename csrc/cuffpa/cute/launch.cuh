...
void launch_cute_fwd_persist_d_fp4_sm120_impl(torch::Tensor Q, torch::Tensor K,
  ...
  const c10::cuda::OptionalCUDAGuard device_guard(Q.device());
  auto stream = at::cuda::getCurrentCUDAStream();
  // hadamard: rotate Q/K copies before the preprocessing chain. Exact in
  // fp32 math (H orthogonal); only moves where quantization noise lands.
  // The rotated copies are kHeadDim-wide (rotated zero pad cols stored),
  // so all downstream consumers stay in one rotated domain; V and the
