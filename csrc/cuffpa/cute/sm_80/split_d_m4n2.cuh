...
// Split-D M4N2 Flash Attention forward (cp.async, sm_80+).
// sm_80 port of the sm120 split-D M4N2 kernel (kBr=64/kBc=64, atom layout
// (4,2,1), O regs = D/4, cross-N-warp softmax exchange, P SMEM roundtrip):
// the TMA loader becomes the sm80 split_d's 16B cp.async chunk pipeline
// (Q/K share a stage pair, V has its own pool) and stmatrix is replaced by
// a vectorized 16B STS on the P staging tile (one swizzle block per store).
// O stays BHND-packed; bias is gmem-direct only (mode 0); dropout is
// inline Philox (PC-14: bitmap measured slower on the 64x64 m4n2 tile).
template <typename Traits, int kHasAttnBias = 0, int kHasDropout = 0>
__global__ void __launch_bounds__(Traits::kNumThreads, 1)
    split_d_m4n2_fwd_cute_sm80(
  ...
  // Cross-N-warp softmax exchange: [8 warps][16 rows] floats (reused
  // max/sum), 16B-aligned past the QK/V/P buffers like the sm120 layout.
  float* smem_exchange = reinterpret_cast<float*>(
      reinterpret_cast<char*>(shm) +
      (((p_base - shm) + kPElements) * (int)sizeof(Element) + 15 & ~15));

  // G2S TiledCopy: 256 threads, 128-bit cp.async (same as the sm80
  // split_d; the 64-wide chunks keep the thread tiling integral).
  ...
  // Dual TiledMma (M4N2 atom layout (4,2,1): 4 M-warps x 2 N-warps).
...
#pragma unroll 1
  for (int kv_tile = 0; kv_tile < Tc_eff; ++kv_tile) {
    // V prefetch for kv_tile > 0 (the split_d cross-tile pattern: this
    // tile's V chunks fly during its QK/softmax).
    if (kv_tile > 0) {
      int v_write = 0;
#pragma unroll
      for (int v = 0; v < kStagesPV - 1 && v < kDChunksV; ++v) {
    ...
    for (int d_chunk = 0; d_chunk < kDChunksQK; ++d_chunk) {
      ...
      if constexpr (kStagesQK == 1) {
        ...
        // stages>=2: issue prefetch (non-blocking, overlaps with gemm).
...
#pragma unroll
      ...
      // Bias tail guards (gmem-direct FC-4 path only).
...
#pragma unroll
      ...
      // Inline philox only (PC-14: bitmap measured slower on m4n2).
      ...
      // Phase 3: P->SMEM in mathematical (m,n) coordinates via a tiled
      // C-copy (AutoVectorizingCopy -> plain STS; the sm_80 has no
      // stmatrix, and a hand-rolled A-operand-layout mapping would not
      // match the LDSM_N read below), then LDSM_N back into the PV A-regs.
      // The write-read barrier doubles as the row-sum publish sync.
      auto tCrP = ffpa_cute::convert_type<Element>(tCrS);
      {
        ...
        auto r2s_copy_p = make_tiled_copy_C(
            Copy_Atom<AutoVectorizingCopy, Element>{}, tiled_mma_qk);
        auto r2s_thr_p = r2s_copy_p.get_slice(tid);
        copy(r2s_copy_p, r2s_thr_p.retile_S(tCrP), r2s_thr_p.partition_D(sP));
      }
      __syncthreads();
        ...
        for (int v_chunk = 0; v_chunk < kDChunksV; ++v_chunk) {
          ...
          if (kv_tile > 0 && need_rescale) {
            auto tCrO_rc = make_tensor(
                tCrO.data(),
                ffpa_cute::convert_layout_acc_rowcol(tCrO.layout()));
#pragma unroll
...
#pragma unroll
          ...
          auto sV = make_tensor(
              make_smem_ptr(v_base + v_read * kVChunkElements), SmemLayoutV{});
  ...
  // Phase 5: Epilogue. O /= row_sum, R->G per v_chunk (BHND rows); only
  // n_warp==0 writes LSE (both N-warps share the same Q rows).
  {
