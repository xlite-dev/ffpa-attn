...
__global__ void __launch_bounds__(Traits::kNumThreads, 1) split_d_fwd_cute_sm80(
  ...
  // area (launcher budgets both). Same half-row scheme as the sm120
  // split_d (kBr=128/kBc=128/256 threads); the fully-synchronous cp.async
  // loop already barriers around each phase, and the explicit sync after
  // apply (below) plus the per-tile phase barriers order the ping-pong
  // buffers' read and regen across iterations.
  ...
  const int bias_area_u16 = (kHasAttnBias && attn_bias_tile_mode != 0)
                                ? 2 * ((attn_bias_tile_mode == 1) ? kBr : 1) *
                                      kBc * ((attn_bias_dtype == 3) ? 2 : 1)
                                : 0;
...
#pragma unroll
    ...
    // Bitmap for the next tile: before this tile's QK/softmax — the
    // cp.async prefetches above are non-blocking, so the gen overlaps them.
