...
#pragma unroll
    ...
    // settling exactly K[t]/V[t] (empty groups complete instantly). The
    // prologue wait_all implicitly commits one extra empty group, so the
    // waits over-settle by one group here (harmless).
    __syncthreads();
    {
