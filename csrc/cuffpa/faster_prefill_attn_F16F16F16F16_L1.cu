...
// FFPA: Faster Flash Prefill Attention, Fine-grained tiling at the MMA level
// for all Q@K^T and P@V results in a constant SRAM usage of Br * 16 or Bc * 16
// for Q, K, V, leading to an overall SRAM complexity of O(Br * 16)~O(1).
// Consequently, this approach allows us to run faster than SDPA w or w/o MMA
// Acc F32. L1: level 1, O(Brx16)~O(1) SRAM complexity, O(d/4) register
// complexity. L2: level 2, O(Brx16)~O(1) SRAM complexity, O(1)   register
// complexity + Q@K^T recomputation. L3: level 3, O(Brx16)~O(1) SRAM complexity,
// O(1)   register complexity + O offloading.
...
__global__ void __launch_bounds__(WARP_SIZE* kMmaTileSeqLenQ* kMmaTileSeqLenK)
    ffpa_mma_stages_split_q_kernel_acc_f16_L1(half* Q, half* K, half* V,
  ...
  constexpr bool kSwizzleQ =
      (kPadQ == 0) ? true : false;  // swizzle Q if kPadQ=0
  constexpr bool kSwizzleK =
      (kPadK == 0) ? true : false;  // swizzle K if kPadK=0
  constexpr bool kSwizzleV =
      (kPadV == 0) ? true : false;  // swizzle V if kPadV=0
  constexpr int Br =
  ...
  // MMA Layout [Br,Bc]=[128,128], MMA = m16n8k16, Br=16x8=128, Bc=8x16=128,
  // layout: 8 warps |  128x128  |      warp_KV 0        | | warp_QP 0 | MMA 0
  // ... MMA 0 (x16) | | warp_QP 1 | MMA 1 ... MMA 1 (x16) | | warp_QP 2 | MMA 2
  // ... MMA 2 (x16) | | warp_QP 3 | MMA 3 ... MMA 3 (x16) | | warp_QP 4 | MMA 4
  // ... MMA 4 (x16) | | warp_QP 5 | MMA 5 ... MMA 5 (x16) | | warp_QP 6 | MMA 6
  // ... MMA 6 (x16) | | warp_QP 7 | MMA 7 ... MMA 7 (x16) |
  ...
  uint32_t R_O[2];  // registers for O=PV[Br,d]=P@V, [4], only use 2 32bits
                    // registers.
          ...
          uint32_t load_smem_Q_ptr =
              (smem_Q_base_ptr +
               (stage * Q_tile_size + load_smem_Q_Br * (kMmaAtomK + kPadQ) +
                (kSwizzleQ ? swizzle_permuted_j<kMmaAtomK>(load_smem_Q_Br,
                                                           load_smem_Q_d + i)
                           : load_smem_Q_d + i)) *
                   sizeof(half));
          ...
          uint32_t load_smem_K_ptr =
              (smem_K_base_ptr +
               (stage * K_tile_size + load_smem_K_Bc * (kMmaAtomK + kPadK) +
                (kSwizzleK ? swizzle_permuted_j<kMmaAtomK>(load_smem_K_Bc,
                                                           load_smem_K_d + i)
                           : load_smem_K_d + i)) *
                   sizeof(half));
...
#pragma unroll
        ...
        // curr K tile g2s
        int load_gmem_K_Bc = (tile_K_seqlen * Bc) + load_smem_K_Bc;  // < seqlen
        int load_gmem_K_d = (tile_K_d * kMmaAtomK) +
                            load_smem_K_d;  // K [Bc,16] from [seqlen,d]
        int load_gmem_K_addr =
            (K_gmem_offset + load_gmem_K_Bc * kHeadDim + load_gmem_K_d);
// swizzle or padding
        ...
        uint32_t lane_smem_Q_ptr =
            (smem_Q_base_ptr +
             (smem_sel * Q_tile_size + lane_smem_Q_Br * (kMmaAtomK + kPadQ) +
              (kSwizzleQ ? swizzle_permuted_j<kMmaAtomK>(lane_smem_Q_Br,
                                                         lane_smem_Q_d)
                         : lane_smem_Q_d)) *
                 sizeof(half));
        ...
        uint32_t lane_smem_K_ptr =
            (smem_K_base_ptr +
             (smem_sel * K_tile_size + lane_smem_K_Bc * (kMmaAtomK + kPadK) +
              (kSwizzleK ? swizzle_permuted_j<kMmaAtomK>(lane_smem_K_Bc,
                                                         lane_smem_K_d)
                         : lane_smem_K_d)) *
                 sizeof(half));
          ...
          uint32_t lane_smem_V_ptr =
              (smem_V_base_ptr +
               (smem_sel_v * V_tile_size +
                ...
                (kSwizzleV ? swizzle_permuted_j<kMmaAtomN * 2>(lane_smem_V_Bc,
                                                               lane_smem_V_d)
                           : lane_smem_V_d)) *
                   sizeof(half));
...
void launch_ffpa_mma_acc_f16_L1(torch::Tensor Q, torch::Tensor K,
  ...
  constexpr int kQKVSmemMaxSize =
      (QK_smem_size > V_smem_size ? QK_smem_size * 2 : V_smem_size * 2);
  ...
  cudaFuncSetAttribute(
      ...
      cudaFuncAttributeMaxDynamicSharedMemorySize, kQKVSmemMaxSize);
  ...
  ffpa_mma_stages_split_q_kernel_acc_f16_L1<
      ...
      kWarpTileSeqLenK, kWarpTileSeqLenP, kWarpTileHeadDimV,
      kOStorageAccFloat32, kStage, kPadQ, kPadK, kPadV>
      <<<grid, block, kQKVSmemMaxSize>>>(reinterpret_cast<half*>(Q.data_ptr()),
