...
//   kHeadDim             Head dimension D; must satisfy D <= 256
//   (static_assert). kMmaAtomM/N/K        MMA atom shape for m16n8k16 (fixed to
//   16/8/16). kMmaTileSeqLenQ/K/P  Warp-level MMA tiling along Q rows / K cols
//   / P rows. kMmaTileHeadDimV     Warp-level MMA tiling along V cols
//   (head-dim). kValTileSeqLenQ/K/P  Value-level multipliers forming Br, Bc as
//   in the
//                        large-d template.
//   kValTileHeadDimV     Value tiling over head-dim for P@V.
...
__global__ void __launch_bounds__(WARP_SIZE* kMmaTileSeqLenQ* kMmaTileSeqLenK)
    ffpa_attn_persistent_d_fwd_template(
        ...
        const unsigned long long philox_seed,
        const unsigned long long philox_offset) {
  // NOTE: This kernel template is developed for small head dimensions (d <=
  // 128), namely, flash-attention-2. Always persist QKV for flash-attn, apply
  // tiling at the Attention level not the MMA level.
  ...
  ffpa::prefill::check_small_d_compiling_states<
      ...
      kValTileSeqLenK, kValTileSeqLenP, kValTileHeadDimV, kMmaAccFloat32QK,
      kMmaAccFloat32PV, kOStorageAccFloat32, kPrefetchQK, kPrefetchPV,
      kShareSmemQKV, kPersistQs2r, kPersistVs2r, kRegPipeKV, kStageQK, kStagePV,
      kPadQ, kPadK, kPadV>();
  constexpr int Br = kMmaAtomM * kMmaTileSeqLenQ * kValTileSeqLenQ;
  constexpr int Bc = kMmaAtomN * kMmaTileSeqLenK * kValTileSeqLenK;
  constexpr int kNumThreads = WARP_SIZE * kMmaTileSeqLenQ * kMmaTileSeqLenK;
...
#pragma unroll
  ...
  // e.g [1][8][2], MMA Acc fp16; [1][8][4], MMA Acc fp32; O=PV[Br,d]=P@V, [4 or
  // 2]
  uint32_t R_S[kValTileSeqLenQ][kValTileSeqLenK][(kMmaAccFloat32QK) ? 4 : 2];
...
#pragma unroll
    for (int tile_K_d = 0; tile_K_d < (kHeadDim / kMmaAtomK); ++tile_K_d) {
      ffpa::prefill::sync_fetch_qkv_frags_s2r<
          0, 4, Q_tile_size, kMmaAtomM, kMmaAtomN, kMmaAtomK, kPadQ, kDataType>(
...
#pragma unroll
      for (int j = 0; j < kValTileHeadDimV; ++j) {
        ...
        if (j % 2 == 0) {
          ffpa::prefill::cp_async_qkv_g2s<Bc, V_tile_size, kHeadDim,
                                          kMmaAtomN * 2, kNumThreads, kPadV>(
              smem_V_base_ptr, V, V_gmem_offset, tile_K_seqlen, tile_V_d,
              tile_V_d, Nkv);
        }
      }
      ffpa::cp_async::commit_group();
...
#pragma unroll
        for (int tile_K_d = 0; tile_K_d < (kHeadDim / kMmaAtomK); ++tile_K_d) {
          ffpa::prefill::cp_async_qkv_g2s<Bc, K_tile_size, kHeadDim, kMmaAtomK,
                                          kNumThreads, kPadK>(
              smem_K_base_ptr, K, K_gmem_offset, (tile_K_seqlen + 1), tile_K_d,
              tile_K_d, Nkv);
        }
    ...
    // T2: {a6, a7}   T3: {a6, a7} 9    T4: {a2, a3}   T5: {a2, a3} T6: {a2, a3}
    // T7: {a2, a3}   T4: {a6, a7}   T5: {a6, a7}   T6: {a6, a7}   T7: {a6, a7}
...
#pragma unroll
          for (int tile_V_Bc = 0; tile_V_Bc < (Bc / kMmaAtomK); ++tile_V_Bc) {
            ffpa::prefill::sync_fetch_qkv_frags_s2r<
                1, 2, V_tile_size, kMmaAtomM, kMmaAtomN, kMmaAtomK, kPadV,
                kDataType>(smem_V_base_ptr, &R_V[tile_V_Bc][0], warp_KV,
                           (j % 2), tile_V_Bc, tile_V_d);
          }
        }
...
#pragma unroll
        for (int tile_V_Bc = 0; tile_V_Bc < (Bc / kMmaAtomK); ++tile_V_Bc) {
          ...
          const int p_offset = tile_V_Bc * 2;  // MMA(Warp) selected, 0, 2, 4, 6
          const int v_offset =
              ((kPersistVs2r) ? tile_V_Bc : ((kRegPipeKV) ? reg_ld_idx : 0));
          if constexpr (kMmaAccFloat32PV) {
            // MMA accumulate with F32 dtype for high precision.
            ffpa::mma::m16n8k16_abf32<kDataType,
                                      ffpa::mma::MMAMode::kInplaceUpdate>(
            ...
            // MMA accumulate with F16 dtype for high throughput.
            ffpa::mma::m16n8k16_f16f16f16<ffpa::mma::MMAMode::kInplaceUpdate>(
                &R_O[0][j][0], &R_O[0][j][1], &R_S[0][p_offset][0],
                &R_S[0][p_offset][1], &R_S[0][p_offset + 1][0],
                &R_S[0][p_offset + 1][1], &R_V[v_offset][0], &R_V[v_offset][1]);
          }
        }  // end for V Bc.
