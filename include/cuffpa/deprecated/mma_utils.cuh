...
#define HMMA16816F32(RD0, RD1, RD2, RD3, RA0, RA1, RA2, RA3, RB0, RB1, RC0, RC1, RC2, RC3)        \
  asm volatile(                                                                                   \
      ...
      : "r"(RA0), "r"(RA1), "r"(RA2), "r"(RA3), "r"(RB0), "r"(RB1), "r"(RC0), "r"(RC1), "r"(RC2), \
        "r"(RC3))
...
#define FFPA_MMA_CHECK_PRINT_REG(R0, R1, format, ...)                                            \
  ...
  if ((fabs(v_reg_0.x - v_reg_1.x) > 0.01f) || (fabs(v_reg_0.y - v_reg_1.y) > 0.01f)) {          \
    printf(format ", R0, V0=%f, V1=%f, R1, V0=%f, V1=%f\n", ##__VA_ARGS__, v_reg_0.x, v_reg_0.y, \
           v_reg_1.x, v_reg_1.y);                                                                \
  }                                                                                              \
  }                                                                                              \
  }
...
#define FFPA_MMA_CHECK_PRINT_T32_REG(R0, R1, format, ...)                                   \
  {                                                                                         \
    if (tid < 32) {                                                                         \
      ...
      float2 v_reg_1 = __half22float2(HALF2(R1));                                           \
      if ((fabs(v_reg_0.x - v_reg_1.x) > 0.01f) || (fabs(v_reg_0.y - v_reg_1.y) > 0.01f)) { \
        printf(format ", R0, V0=%f, V1=%f, R1, V0=%f, V1=%f\n", ##__VA_ARGS__, v_reg_0.x,   \
               v_reg_0.y, v_reg_1.x, v_reg_1.y);                                            \
      }                                                                                     \
    }                                                                                       \
  }
