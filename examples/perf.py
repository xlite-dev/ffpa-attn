...
CUTEDSL_SECTION_LABEL = "CuTeDSL (SM90 256<D<=512)"
...
def render_speedup_markdown(
  ...
  *,
  ...
  if cutedsl:
    lines.extend([
      "",
      "Backend: CuTeDSL SM90 dense 256<D<=512 path (fp16/bf16 forward, bf16-only backward). "
...
def main() -> None:
  ...
  if is_cutedsl:
    ...
    if args.D <= 256 or args.D > 512:
      raise SystemExit(
        f"[cutedsl] --D must satisfy 256 < D <= 512; got {args.D}."
      )
    _require_sm90()
