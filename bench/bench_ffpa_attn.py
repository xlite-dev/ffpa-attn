...
def run_benchmark(
  ...
  if "sdpa" in tag:
    SDPA_TFLOPS = TFLOPS
  out_info = f"{tag}"
  # Cast to float32 first so bf16 tensors can be converted via numpy.
...
def plot_speedup_bar(speedup: list[float], extra_tag: str = "ffpa+acc+f32", headdim: list[int] = None):
  ...
  if args.save_tag:
    save_path = (f"{args.save_dir}/{device_name}_{args.save_tag}_{extra_tag}_Speedup.png")
  else:
...
def get_qkvo(B, H, N, D):
  t_dtype = torch.bfloat16 if args.dtype == "bf16" else torch.half
  if not (args.no_rand_q or args.no_rand_qkv):
    q = torch.randn((B, H, N, D), dtype=t_dtype, device="cuda")
  else:
    q = torch.ones(B, H, N, D, device="cuda", dtype=t_dtype).contiguous()
  if not (args.no_rand_k or args.no_rand_qkv):
    k = torch.randn((B, H, N, D), dtype=t_dtype, device="cuda")
  else:
    k = torch.ones(B, H, N, D, device="cuda", dtype=t_dtype).contiguous()
  if not (args.no_rand_v or args.no_rand_qkv):
    v = torch.randn((B, H, N, D), dtype=t_dtype, device="cuda")
  else:
    v = torch.ones(B, H, N, D, device="cuda", dtype=t_dtype).contiguous()

  o = torch.zeros(B, H, N, D, device="cuda", dtype=t_dtype).contiguous()
...
def check_all_close(
  ...
  # bf16 has ~3 bits less mantissa than fp16; relax tolerance accordingly.
  atol = 2e-2 if args.dtype == "bf16" else 1e-2
  all_close = str(torch.allclose(out_flash_or_sdpa, out_mma, atol=atol))
  pretty_print_line(
