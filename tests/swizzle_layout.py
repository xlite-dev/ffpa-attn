...
def swizzle_permuted_j(
  ...
  return ((int(j / num_elems_per_128b) ^ int(i / 4)) %
          (int(col_stride / num_elems_per_128b))) * num_elems_per_128b
...
def print_smem_swizzle_layout(
  ...
  for i in range(rows):
    ...
    if i == 0:
      ...
      pretty_print_line(
        f"smem col 0~16, step {num_elems_per_128b}" if logical_col_stride >= 16
