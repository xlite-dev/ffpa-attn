...
def test_fp4_mxfp8_pv_rejects_large_d(monkeypatch):
  monkeypatch.setenv("FFPA_CUDA_ALLOW_SMALL_D", "1")
  torch.manual_seed(0)
  B, H, N, D = 1, 32, 2048, 256  # 256 > 192: MXFP8 PV is smem-limited
  q, k, v = _mk(B, H, H, N, D)
...
def test_fp4_smooth_v_rejects_split_d():
  torch.manual_seed(0)
  B, H, N, D = 1, 32, 2048, 320  # split-D: smooth_v is persist-D only
  q, k, v = _mk(B, H, H, N, D)
  with pytest.raises(RuntimeError, match="persist_d"):
    with torch.no_grad():
      ffpa_attn_func(q, k, v, forward_backend=_fp4_backend(fp4_smooth_v=True))
