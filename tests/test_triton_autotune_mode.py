...
def test_split_launch_accepts_without_backward_tma():
  meta = FFPAAttnMeta.from_backends(
    backward_backend=TritonBackend(backward=True, split_launch=True)
  )
  assert isinstance(meta.backward_meta, TritonBackend)
  assert meta.backward_meta.enable_tma is False
...
def test_split_launch_accepts_backward_tma():
  meta = FFPAAttnMeta.from_backends(
    backward_backend=TritonBackend(
      backward=True, enable_tma=True, split_launch=True
    ),
  )
  assert isinstance(meta.backward_meta, TritonBackend)
  assert meta.backward_meta.enable_tma is True
...
def test_from_options_rejects_string_backends():
  with pytest.raises(
    TypeError, match="forward_backend must be a Backend object"
  ):
    FFPAAttnMeta.from_options(
...
def test_new_backends_reject_wrong_direction_pairing():
  with pytest.raises(
    AssertionError, match="forward_backend must be configured"
  ):
    FFPAAttnMeta.from_backends(
      TritonBackend(backward=True), TritonBackend(backward=True)
    )
...
def test_sdpa_backward_exposes_high_precision_grad():
  meta = FFPAAttnMeta.from_backends(
    backward_backend=SDPABackend(backward=True, high_precision_grad=True)
  )
...
def test_fwd_fast_mode_prunes_decode_configs():
  fast = _gen_decode_fwd_stage1_autotune_configs(
    320, use_gemv=False, autotune_mode="fast"
  )
  max_configs = _gen_decode_fwd_stage1_autotune_configs(
    320, use_gemv=False, autotune_mode="max"
  )
  assert len(fast) < len(max_configs)
...
def test_sm90_fwd_configs_force_warp_specialize():
  ...
  max_configs = _gen_fwd_sm90_autotune_configs(
    autotune_mode="max", enable_ws=True
  )
  ...
  max_ws_configs = [
    config for config in max_serialized if config["warp_specialize"]
  ]
  assert len(fast) < len(max_configs)
...
def test_sm90_fwd_max_configs_add_full_headdim_on_large_gpu(monkeypatch):
  ...
  monkeypatch.setattr(
    torch.cuda, "get_device_name", lambda device=0: "NVIDIA H800"
  )
...
def test_sm90_fwd_max_configs_skip_full_headdim_on_rtx(monkeypatch):
  ...
  monkeypatch.setattr(
    torch.cuda, "get_device_name", lambda device=0: "NVIDIA GeForce RTX 5090"
  )
...
def test_persistent_tune_forward_records_sm90_tma_config(monkeypatch):
  ...
  generic_wrapper = SimpleNamespace(
    best_config=generic_config, configs=[generic_config]
  )
  ...
  monkeypatch.setattr(
    autotune_module, "_make_tensors", lambda task, batch: (q, k, v)
  )
  monkeypatch.setattr(autotune_module, "_make_attn_bias", lambda task: None)
  monkeypatch.setattr(
    autotune_module, "_get_decode_num_splits", lambda *args, **kwargs: 1
  )
  monkeypatch.setattr(
    autotune_module, "is_sm90_tma_forward_supported",
    lambda *args, **kwargs: True
  )
  monkeypatch.setattr(autotune_module, "ffpa_attn_func", fake_ffpa_attn_func)
  monkeypatch.setattr(
    autotune_module, "_get_fwd_autotune",
    lambda *args, **kwargs: generic_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_fwd_sm90_autotune",
    lambda *args, **kwargs: sm90_wrapper
  )

  entries = {}
  tuned_entries = _tune_forward(
    task, 1, "fast", entries, enable_tma=True, enable_ws=True
  )

  assert [entry["kernel"]
          for entry, _ in tuned_entries] == ["fwd_generic", "fwd_sm90_generic"]
...
@pytest.mark.parametrize(
  ...
  [
    ...
    TuneTask(
      "forward", torch.float16, 320, 512, 512, False, 8, 8, has_attn_bias=True
    ),
    TuneTask(
      "forward", torch.float16, 320, 512, 512, False, 8, 8, has_dropout=True
    ),
  ],
)
def test_persistent_tune_forward_allows_sm90_ws_for_masked_variants(
  monkeypatch, task
):
  q = torch.empty(1, 8, 512, 320)
  k = torch.empty_like(q)
  ...
  monkeypatch.setattr(
    autotune_module, "_make_attn_bias", lambda task: torch.empty(1)
    if task.has_attn_bias else None
  )
  monkeypatch.setattr(
    autotune_module, "_get_decode_num_splits", lambda *args, **kwargs: 1
  )
  ...
  monkeypatch.setattr(autotune_module, "ffpa_attn_func", fake_ffpa_attn_func)
  monkeypatch.setattr(
    autotune_module, "_get_fwd_autotune", lambda *args, **kwargs: wrapper
  )
...
def test_bwd_non_main_modes_use_same_decode_configs(use_gemv):
  fast = _gen_decode_bwd_stage1_autotune_configs(
    512, use_gemv=use_gemv, autotune_mode="fast"
  )
  max_configs = _gen_decode_bwd_stage1_autotune_configs(
    512, use_gemv=use_gemv, autotune_mode="max"
  )
  assert [config_from_triton_config(config) for config in fast
          ] == [config_from_triton_config(config) for config in max_configs]
...
def test_sm90_bwd_tma_configs_are_tma_only_in_phase1(monkeypatch):
  ...
  fast = _gen_bwd_sm90_autotune_configs(
    512, autotune_mode="fast", enable_ws=False
  )
  max_configs = _gen_bwd_sm90_autotune_configs(
    512, autotune_mode="max", enable_ws=False
  )
  serialized = [config_from_triton_config(config) for config in fast]
  max_serialized = [config_from_triton_config(config) for config in max_configs]
...
def test_sm90_bwd_tma_configs_force_warp_specialize(monkeypatch):
  ...
  fast = _gen_bwd_sm90_autotune_configs(
    512, autotune_mode="fast", enable_ws=True
  )
  max_configs = _gen_bwd_sm90_autotune_configs(
    512, autotune_mode="max", enable_ws=True
  )
  serialized = [config_from_triton_config(config) for config in fast]
  max_serialized = [config_from_triton_config(config) for config in max_configs]
...
def test_sm90_bwd_split_configs_use_split_launch_autotune_flag(monkeypatch):
  ...
  dkdv_fast = _gen_bwd_sm90_dkdv_autotune_configs(
    512, autotune_mode="fast", enable_ws=False
  )
  ...
  dq_fast = _gen_bwd_sm90_dq_autotune_configs(
    512, autotune_mode="fast", enable_ws=False
  )
  dq_max = _gen_bwd_sm90_dq_autotune_configs(
    512, autotune_mode="max", enable_ws=True
  )
  dkdv_serialized = [config_from_triton_config(config) for config in dkdv_fast]
  dkdv_persist_serialized = [
    config_from_triton_config(config) for config in dkdv_persist_fast
  ]
  dkdv_persist_max_serialized = [
    config_from_triton_config(config) for config in dkdv_persist_max
  ]
  fused_persist_max_serialized = [
    config_from_triton_config(config) for config in fused_persist_max
  ]
  split_persist_max_serialized = [
    config_from_triton_config(config) for config in split_persist_max
  ]
  ...
  assert {config["BLOCK_HEADDIM"]
          for config in dkdv_persist_serialized} == {128}
  assert {config["BLOCK_HEADDIM"]
          for config in dkdv_persist_max_serialized} == {128, 256}
  assert {config["BLOCK_HEADDIM"]
          for config in fused_persist_max_serialized} == {64, 128}
  assert {config["BLOCK_HEADDIM"]
          for config in split_persist_max_serialized} == {128, 256}
...
def test_persistent_tune_backward_records_sm90_tma_config(monkeypatch):
  ...
  monkeypatch.setattr(autotune_module, "_make_attn_bias", lambda task: None)
  monkeypatch.setattr(
    autotune_module, "is_sm90_tma_backward_supported",
    lambda *args, **kwargs: True
  )
  monkeypatch.setattr(autotune_module, "ffpa_attn_func", fake_ffpa_attn_func)
  monkeypatch.setattr(
    autotune_module, "_get_pre_autotune", lambda *args, **kwargs: pre_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_bwd_autotune",
    lambda *args, **kwargs: generic_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_bwd_sm90_autotune",
    lambda *args, **kwargs: sm90_wrapper
  )

  entries = {}
  tuned_entries = _tune_backward(
    task, 1, "fast", entries, enable_tma=True, enable_ws=False
  )

  assert [entry["kernel"] for entry, _ in tuned_entries
          ] == ["bwd_preproc", "bwd_generic", "bwd_sm90_generic"]
...
def test_persistent_tune_backward_records_split_sm90_tma_configs(monkeypatch):
  ...
  monkeypatch.setattr(
    autotune_module, "_get_bwd_dkdv_autotune",
    lambda *args, **kwargs: generic_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_bwd_dq_autotune",
    lambda *args, **kwargs: generic_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_bwd_sm90_autotune",
  ...
  monkeypatch.setattr(
    autotune_module, "_get_bwd_sm90_dkdv_autotune",
    lambda *args, **kwargs: dkdv_wrapper
  )
  monkeypatch.setattr(
    autotune_module, "_get_bwd_sm90_dq_autotune",
    lambda *args, **kwargs: dq_wrapper
  )
  ...
  assert [kwargs["backward_backend"].split_launch
          for kwargs in seen_kwargs] == [False, True, False, True]
  assert seen_kwargs[-1]["backward_backend"].enable_tma is True


def test_persistent_tune_backward_records_generic_split_without_tma(
  monkeypatch
):
  task = TuneTask("backward", torch.float16, 320, 512, 512, False, 8, 8)
  q = torch.empty(1, 8, 512, 320, requires_grad=True)
  ...
  assert [kwargs["backward_backend"].enable_tma
          for kwargs in seen_kwargs] == [False, False]
  assert [kwargs["backward_backend"].split_launch
          for kwargs in seen_kwargs] == [False, True]


def test_forward_autotune_keys_include_causal():
  expected_keys = {
    "autotune_seqlen_q_bucket", "autotune_seqlen_k_bucket",
    "autotune_causal_key"
  }
  assert expected_keys <= set(_get_fwd_autotune(320, "fast", "bf16").keys)
  assert expected_keys <= set(
    _get_fwd_sm90_autotune(320, "fast", "bf16", enable_ws=False).keys
  )
  assert expected_keys <= set(
    _get_decode_fwd_stage1_autotune(320, True, "fast", "bf16").keys
  )
  ...
  assert expected_keys <= set(
    _get_bwd_sm90_autotune(320, "fast", "bf16", False, enable_ws=False).keys
  )
  assert expected_keys <= set(
    _get_bwd_sm90_dkdv_autotune(320, "fast", "bf16", False,
                                enable_ws=False).keys
  )
  assert expected_keys <= set(
    _get_bwd_sm90_dq_autotune(320, "fast", "bf16", enable_ws=False).keys
  )


def test_sm90_bwd_persist_autotune_is_cache_scoped():
  normal = _get_bwd_sm90_autotune(
    320, "fast", "bf16", False, enable_ws=False, enable_persist_dkdv=False
  )
  persist = _get_bwd_sm90_autotune(
    320, "fast", "bf16", False, enable_ws=False, enable_persist_dkdv=True
  )
  ws = _get_bwd_sm90_autotune(
    320, "fast", "bf16", False, enable_ws=True, enable_persist_dkdv=False
  )
  persist_ws = _get_bwd_sm90_autotune(
    320, "fast", "bf16", False, enable_ws=True, enable_persist_dkdv=True
  )
...
def test_sm90_bwd_split_autotune_is_cache_scoped():
  dkdv = _get_bwd_sm90_dkdv_autotune(
    320, "fast", "bf16", False, enable_ws=False, enable_persist_dkdv=False
  )
  dkdv_persist = _get_bwd_sm90_dkdv_autotune(
    320, "fast", "bf16", False, enable_ws=False, enable_persist_dkdv=True
  )
  dkdv_ws = _get_bwd_sm90_dkdv_autotune(
    320, "fast", "bf16", False, enable_ws=True, enable_persist_dkdv=False
  )
...
def test_autotune_wrappers_are_dtype_scoped():
  assert _get_fwd_autotune(320, "fast", "bf16"
                           ) is not _get_fwd_autotune(320, "fast", "fp16")
  assert _get_fwd_sm90_autotune(320, "fast", "bf16",
                                enable_ws=False) is not _get_fwd_sm90_autotune(
                                  320, "fast", "fp16", enable_ws=False
                                )
  assert _get_decode_fwd_stage1_autotune(
    320, True, "fast", "bf16"
  ) is not _get_decode_fwd_stage1_autotune(320, True, "fast", "fp16")
  assert _get_pre_autotune(False, "fast", "bf16"
                           ) is not _get_pre_autotune(False, "fast", "fp16")
...
def test_persistent_autotune_decode_tasks_skip_nkv1_and_nq4():
  ...
  forward_decode_tasks = [
    task for task in _iter_forward_tasks(dtypes, seqlens) if task.seqlen_q == 1
  ]
  backward_decode_tasks = [
    task for task in _iter_backward_tasks(dtypes, seqlens) if task.seqlen_q < 8
  ]

  assert forward_decode_tasks
  assert backward_decode_tasks
  assert all(
    task.seqlen_q == 1 and task.seqlen_k > 1 for task in forward_decode_tasks
  )
  assert all(
    task.seqlen_q == 1 and task.seqlen_k > 1 for task in backward_decode_tasks
  )
...
def test_persistent_autotune_tasks_start_with_common_prefill():
  ...
  backward_tasks = _iter_backward_tasks(
    dtypes, seqlens, heads=8, full_tasks=True
  )
  ...
  first_forward_full = next(
    index for index, task in enumerate(forward_tasks)
    if task.case_name == "attn-mask"
  )
  first_backward_full = next(
    index for index, task in enumerate(backward_tasks)
    if task.case_name == "attn-mask"
  )
  assert any(
    task.case_name == "decode-attn"
    for task in forward_tasks[:first_forward_full]
  )
  assert any(
    task.case_name == "decode-attn"
    for task in backward_tasks[:first_backward_full]
  )
...
def test_persistent_autotune_default_tasks_keep_baseline_variants():
  ...
  tasks = _iter_forward_tasks(dtypes, seqlens, heads=8) + _iter_backward_tasks(
    dtypes, seqlens, heads=8
  )
...
def test_persistent_autotune_full_tasks_add_mask_dropout_gqa_mqa():
  ...
  assert any(
    task.case_name == "attn-mask" and task.has_attn_bias for task in tasks
  )
  assert any(task.case_name == "dropout" and task.has_dropout for task in tasks)
  assert any(
    task.case_name == "gqa" and task.nheads_q == 8 and 1 < task.nheads_kv < 8
    for task in tasks
  )
  assert any(
    task.case_name == "mqa" and task.nheads_q == 8 and task.nheads_kv == 1
    for task in tasks
  )
