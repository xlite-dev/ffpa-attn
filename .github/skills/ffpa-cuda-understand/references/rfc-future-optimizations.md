...
- [ ] PC-0：attn mask 场景性能优化 —— bias tile IO 重构（P 轨最高优先，2026-08-31 立项）
  ...
  - [x] PC-0-5-pd：fp4 persist_d 独立 epilogue race 复测与定性（2026-09-07 完成，**定性收敛、代码不动**）
    ...
    - 缓解选项（未落地，待决策）：A) mode 0 dense 注入向量化（PC-0-2 dense 延伸，减少停顿次数改变时序，修复+性能双赢候选，但需机制解释防"时序副产物修复"）；B) fp4 persist_d + dense fp32 demote 决策改为物化 fp16/bf16 + mode 1——**对 0/1 二值 mask（最常见形态）零精度损失**（0/1 均为 fp16 精确值），且 fp16 dense 在 D=128/D=256 实测均走 mode 1 且 load=400 下 0/40×2 免疫（`.tmp/pc5-race/persist_d_fp16_allD.py`，D=64/192 未编译未测）；C) 维持现状文档化（触发面窄）。复现脚本：`.tmp/pc5-race/persist_d_{rate,self,mode,nobias,burst,workbench,dissect,kname,load,dtype,fp8ctl,san100,fp16_allD}.py`。
