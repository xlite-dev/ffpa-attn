...
| PC-0-5 | ↳ 量化家族先在时序竞争排查（PC-0-1 B-4/B-6 实证，bitwise 断言 flaky 根因） | P3 | 🚧 深度排查完成、根因定性（fp4 m4n2：触发面 = **bias 数据经 smem**——2026-09-02 mode 矩阵实锤：mode 0（gmem 直读，无 smem bias 缓冲）**完全免疫** 0/6×3，mode 2（TMA row-broadcast smem）/mode 3（装载段 fill 写 smem）均 6/6 必触发（模板名 profiler 实证 Li2E/Li3E）；C9b 同模板（Li3E）零 bias prelude → 随机 bias target 仍 6/6 → **跨模板切换非必要条件**（旧"双必要条件"中 no-bias 模板前置一条修正为"前置调用与 target 存在输入差异"）；racecheck 0 hazard；bias 数值无关（C2a）；barrier 语义修复无效（A2/A3/B）→ 定性为 smem bias 写路径相关的时序敏感竞争，疑似 sm_120 TMA/L2 与调度交互的硬件/driver 层问题。无条件 barrier（A）可修但致 ptxas spill 翻倍、mode 3 慢 57%（SASS 实证）→ 不可用。**已落地**：tail work 的 `tma_store_wait` 协议补全（B，性能无回退）+ E1 清理 + race 用例沉淀 tests（xfail）+ `FFPA_BIAS_RESIDENT_DISABLE` A-B 开关。**缓解路径（性能换确定性）**：`FFPA_BIAS_TILE_DISABLE=1` 强制 mode 0 完全免疫（代价 = 丢 PC-0-1 attn-mask 收益，仅作 escape hatch 不默认）。后续：NVIDIA 上报（最小复现器 `.tmp/pc5-race/m4n2_e5d.py`，mode 矩阵证据链完整）；fp4 persist_d 3/30 触发待排查（persist_d 无 mode 0 等价路径，若同族则需独立解）；fp8 六族实证干净 | PC-0-1；PC-11 解耦可独立推进 |
...
- [ ] PC-0：attn mask 场景性能优化 —— bias tile IO 重构（P 轨最高优先，2026-08-31 立项）
  ...
  - [ ] PC-0-5：量化家族先在时序竞争排查（2026-09-01 深度排查完成、根因定性；2026-09-02 触发面收窄 + 确认缓解路径）
    ...
    - **触发面（2026-09-02 mode 矩阵收窄，修正 09-01 双必要条件）**：共同条件 = **bias 数据经 smem**——mode 0（gmem 直读，无 smem bias 缓冲，`FFPA_BIAS_TILE_DISABLE=1`）**完全免疫**（p0 0/6 ×2 轮 + C9b 变体 0/6）；mode 2（TMA row-broadcast 双缓冲写 smem，`FFPA_BIAS_RESIDENT_DISABLE=1`）**6/6 ×2 轮**（模板名 profiler 实证 Li2ELi0ELi1E，排除 stale build）；mode 3（装载段 fill 写 4096B STS）6/6。**C9b 排除跨模板切换**：prelude 与 target 同模板（Li3E）仅 bias 数值不同（全零 → 随机）仍 6/6 → 旧条件①"no-bias 模板前置"修正为"前置调用与 target 存在输入/参数差异"；触发与写方式（TMA store vs fill STS）无关，只与"bias 进 smem"相关。剂量效应保留（C7 单写 2B 稳定）：smem bias 写需达一定规模才打开扰动窗口。
    - 消元链（C1-C9，09-01）：bias 数值无关（常数填充仍触发 C2a）、staging/TMA 复用无关（强制 R2G 仍触发 C1）、注入调用非必要（禁用后 3/6 C4）、只留 barrier=稳（C6）、删装载段=稳（C5，与 mode 0 免疫自洽）。
    - 排除项（全部实证）：smem barrier 语义缺失（racecheck 0 hazard 且 race 在 hook 下复现；tile-0 barrier 语义等价注入 A2/A3 均无效）、插桩伪影链、共享量化链（E5c split_d 同进程稳定）、越界写（launcher printf 实证 56320+4096=60416B 合法）、未初始化读（initcheck 0）。
    - 修复探索：**候选 A（kv loop 顶无条件 barrier）修复 race 但 mode 3 慢 57%**（SASS diff：spill 822 vs 440 次/tile）；"修复"是 spill 时序副产物（A3 语义等价却不修）→ 拒绝。**已落地 B**：tail work `tma_store_wait` 无条件化 + `__threadfence`（协议补全，性能无回退）+ E1 清理 + race 用例沉淀 `tests/test_ffpa_fp4_m4n2_bias_race.py`（xfail）+ `FFPA_BIAS_RESIDENT_DISABLE` A-B 开关（launch.cuh fp4 m4n2 跳过 mode 2→3 升级）。
    - 定性与后续：**smem bias 写路径相关的时序敏感竞争**（语言级协议完备、sanitizer 不可见、时序扰动可调制触发率）——疑似 sm_120 TMA/L2 与调度交互的硬件/driver 层问题。**缓解路径（确定可用）**：`FFPA_BIAS_TILE_DISABLE=1` 退 mode 0 完全免疫，代价 = 丢 PC-0-1 attn-mask 收益（fp4 split_d D=320 1.67x 等）→ 仅作 escape hatch，不默认（触发面窄：fp4 m4n2 bias 场景 + 前置差异调用，数值影响 bitwise 级）。根治需 NVIDIA 层面：最小复现器 + mode 矩阵证据链上报；fp4 persist_d 3/30 待排查（persist_d 无 mode 0 等价路径，若同族需独立解）；fp8 六族实证干净。
