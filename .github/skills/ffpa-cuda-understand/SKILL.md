...
**sm89（Ada）fp8 支持**（2026-09-11 落地）：sm_89 无 TMA/async proxy，只能 cp.async general proxy。fp8 家族在 sm89 GPU（arch major < 12）或 `FFPA_FP8_SM89_FORCE=1` 下分流到 `cute/fp8/sm_89/persist_d.cuh`（见 §5.1）。v1 scope：**persist-D**（D≤224）、per_block Q/K/V、int8 QK + f16 PV acc（与 sm120 默认配置一致的量化配方）；**per-thread QK / per-channel V（+smooth_v）规划为 FC-14**（RFC 登记 2026-09-20，设计稿就绪）；split-D/M4N2 sm89 待 Phase 2/3（功能正确即可，低优先）。
...
> **sm89 vs sm120 关键差异**：sm89 无 TMA/mbarrier/WS，PV 用 SM89 专用 f16-acc atom（traits 的 SM120 atom 是 Blackwell 专属）；V^T 装载沿用 sm80 64 列段（D%64==0 gate）。v1 scope 严格：per_block Q/K/V、f16 PV acc、`q_start_row==0`、`!smooth_v`、`!dropout`、NHD-O 拒（`TORCH_CHECK` 响亮报错）——**per_thread QK / per_channel V / smooth_v 见 FC-14（RFC 登记，待实施）**。已验证（2026-09-20 全 task 矩阵，`.tmp/sm89_perf/bench5_compare.py`）：**6/6 bitwise 一致**（dense/causal/cross/gqa/unaligned @N=8192 + causal@16384，跨进程 dump/compare）；**性能平价全 task 成立**——e2e 与 attn-kernel 级差距均 ≤0.35%（dense 3150.0/3152.4、causal 1865.9/1861.4、cross 701.0/701.1、gqa 2948.8/2949.5、unaligned 3165.1/3168.7µs，attn kernel 2764.5/2767.5 等）——**cp.async 与 TMA 装载在 kBr=64/2CTA 几何下等价**（装载非瓶颈，两 arch 同源瓶颈 = softmax/发射链），vs Sage2 见上方 PC-17 终态段。
>
...
| knob | 取值 | 支持范围 |
|---|---|---|
| `fp8_q/k_quant_method` | `per_block` / `per_thread`（Q/K 必须同配置） | sm120 三族 ✓；sm89 persist-D 现仅 `per_block`，`per_thread` 见 FC-14 |
| `fp8_v_quant_method` | `per_block` / `per_channel` | sm120 三族 ✓（per-channel stats/quantize 均 D_og 感知）；sm89 persist-D 现仅 `per_block`，`per_channel`（含 smooth_v）见 FC-14 |
...
|---|---|---|---|
| native（AUTO/NATIVE/TMA） | 编译集（默认 = default 集 {64,128,192,256,320,512}；`--headdim all` {64,128,192,256,320,512,768}） | `D_og%8==0` → **64 对齐** ∈[64,1024]（FC-8） | **Q/K/V 不物化**：sm80 cp.async 16B chunk 列守卫（`cp_async_zfill` src-size=0，含 decode split-KV）/ sm90+ TMA descriptor `minor_dim=d_og` OOB 零填充；仅 O pad 切回。TMA hint 仅在 TMA ext 已编译且 sm90+ 计入（pre-sm90 回落 CUTE sm80 走 32 对齐物化 pad） |
...
# 不传 flag = 'default'（64,128,192,256,320,512）
# 'default' → 64,128,192,256,320,512；'all' → default + 768（832-1024 编译极慢，已移出 all）
# 其它 headdim（32/96/120/224/832..1024 等）必须显式列表传；ENABLE_FFPA_ALL_HEADDIM=1 env（仅裸 setup.py/pip 路径）可编满 [64,1024] %64
