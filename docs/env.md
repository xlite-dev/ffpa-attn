...
- <span style="color:#c77dff;">ENABLE_FFPA_FWD_CUDA_IMPL</span>, default `False (0)`, Build the optional native CUDA forward extension (`ffpa_attn._C`). When unset, the package builds in pure Python / Triton-only mode and large-D forward defaults to the Triton backend.
- <span style="color:#c77dff;">ENABLE_FFPA_BWD_CUDA_IMPL</span>, default `False (0)`, Build the optional native CUDA Split-D backward implementation. This flag is only effective when `ENABLE_FFPA_FWD_CUDA_IMPL=1`; if CUDA forward is disabled, CUDA backward is forcibly disabled as well.
- <span style="color:#c77dff;">FFPA_SKIP_CUDA_EXT</span>, default `False (0)`, Force-skip the CUDA extension build even if `ENABLE_FFPA_FWD_CUDA_IMPL=1` is set. Used by ReadTheDocs and the `check-mkdocs` GitHub workflow where no nvcc is available; mkdocstrings can still import `ffpa_attn` to extract docstrings.
...
3. **Default build mode**: source builds are Triton-only unless `ENABLE_FFPA_FWD_CUDA_IMPL=1` is set. `FFPA_SKIP_CUDA_EXT=1` is now only an override that force-disables CUDA extension builds even when the CUDA build flags are enabled.
4. **Default arch inference**: when `FFPA_BUILD_ARCH` is unset and `ENABLE_FFPA_FWD_CUDA_IMPL=1`, `setup.py` queries the currently visible CUDA device's compute capability. On CI / RTD runners with no GPU, either keep the default Triton-only build or explicitly set `FFPA_SKIP_CUDA_EXT=1`.
