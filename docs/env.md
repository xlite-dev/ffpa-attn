...
2. **Build-time vs. runtime separation**: `FFPA_BUILD_STAGES`, `ENABLE_FFPA_ALL_STAGES` and `ENABLE_FFPA_ALL_HEADDIM` decide which TUs are generated at build time — changing them requires rebuilding (`FFPA_CLEAN=1 bash tools/build_fast.sh`). All other `ENABLE_FFPA_*` knobs are read at runtime and can be toggled without rebuilding.
