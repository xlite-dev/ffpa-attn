...
5. **Recommended for tuning**: enable `FFPA_PTXAS_VERBOSE=1` (build-time) and start from the defaults; toggle one runtime knob at a time and measure with `bench/bench_ffpa_fwd.py` to attribute the speedup or regression.
