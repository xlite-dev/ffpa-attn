...
- <span style="color:#c77dff;">ENABLE_FFPA_ALL_HEADDIM</span>, default `False (0)`, When `1`, sets `-DENABLE_FFPA_ALL_HEADDIM` (a C++ compile flag). Both `0` and `1` generate headdims that are multiples of 64 in `[64, 1024]` (`range(64, 1024, 64)`). Pass non-64-multiple headdims (e.g. 32, 96) via `FFPA_DEV_HEADDIMS`.
